# Remove `packages/talon/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/talon/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the talon legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/talon/src/storage/db.ts` are concentrated in 22 source files:

- `packages/talon/src/account/repo.ts`
- `packages/talon/src/cli/cmd/db.ts`
- `packages/talon/src/cli/cmd/import.ts`
- `packages/talon/src/cli/cmd/stats.ts`
- `packages/talon/src/control-plane/workspace.ts`
- `packages/talon/src/index.ts`
- `packages/talon/src/node.ts`
- `packages/talon/src/permission/index.ts`
- `packages/talon/src/project/project.ts`
- `packages/talon/src/server/projectors.ts`
- `packages/talon/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/talon/src/server/shared/fence.ts`
- `packages/talon/src/session/message-v2.ts`
- `packages/talon/src/session/projectors.ts`
- `packages/talon/src/session/prompt.ts`
- `packages/talon/src/session/session.ts`
- `packages/talon/src/session/todo.ts`
- `packages/talon/src/share/share-next.ts`
- `packages/talon/src/storage/db.ts`
- `packages/talon/src/sync/index.ts`
- `packages/talon/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy talon database wrapper; `packages/talon/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the talon wrapper.
...
Status: Completed. `SyncEvent` and the talon projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy talon database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy talon database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy talon database wrapper.
...
All migration groups are complete or superseded. `packages/talon/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No talon data-migration group remains.

The previous talon `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate talon data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/talon`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/talon/src`
- `bun typecheck` from `packages/talon`
- Relevant package tests from `packages/talon`, not the repo root
