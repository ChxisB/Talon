...
The talon server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@talon-ai/llm` tool loop after replacing its
...
- plugins get global "talon" instance like in that post i showed
- talon instance has stuff like `talon.session.prompt()` or
  `talon.tool.register({...})`
...
- decide whether to preserve deprecated `@talon-ai/llm` orchestration exports
