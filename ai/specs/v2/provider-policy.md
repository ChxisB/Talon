...
Both `action` and `resource` use talon's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.talon` files is intentionally deferred until `.talon` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether talon permits an operation through its managed execution paths.
