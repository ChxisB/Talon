...
`packages/talon/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/talon ---\
                      > @talon-ai/tui -> @talon-ai/sdk
packages/cli --------/
```

The TUI may directly depend on terminal and UI infrastructure such as
`@tui/core`, `@tui/solid`, `@tui/keymap`, `solid-js`, Effect, and
generic presentation libraries. It must not depend on `packages/talon`,
`packages/cli`, or `@talon-ai/core`.
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/talon`.
- Do not replace private `packages/talon` imports with `@talon-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@talon-ai/tui`.
...
```text
packages/talon/src/cli/cmd/tui
```

Its private dependency on `packages/talon` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/talon/src/*`.
...
Status: Completed. The private `@talon-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@talon-ai/tui`.
...
- The package has no dependency on `talon`, `@talon-ai/cli`, or
  `@talon-ai/core`.
...
primitives now live in `@talon-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@talon-ai/tui/runtime`. Movable app, component,
...
`@talon-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/talon` initially.
...
canonical in `@talon-ai/tui`. Configured references resolve through the new
...
`@talon-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/talon`.
...
- The package has no imports from `packages/talon`, `packages/cli`, or
  `@talon-ai/core`.
...
public `@talon-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/talon`.
- Keep the legacy embedded worker and server startup in `packages/talon`.
- Change those adapters to load config, create transport inputs, and call the
  public `@talon-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/talon`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@talon-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@talon-ai/core`, or either
  executable package.
...
cd packages/talon && bun typecheck
...
rg '@talon-ai/core|packages/talon|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/talon/src packages/talon/test
```
...
- Build the current-platform `packages/talon` binary.
