import { InputRenderable, TextareaRenderable, type CliRenderer, type KeyEvent, type Renderable } from "@tui/core"
import {
  ...
  registerTimedLeader,
} from "@tui/keymap/addons/tui"
import { stringifyKeyStroke, type Binding } from "@tui/keymap"
import {
  formatCommandBindings as formatCommandBindingsExtra,
  formatKeySequence as formatKeySequenceExtra,
} from "@tui/keymap/extras"
import { KeymapProvider, useKeymap, useKeymapSelector, useBindings } from "@tui/keymap/solid"
import { createMemo, type Accessor } from "solid-js"
import { useTuiConfig } from "./config"
import { TuiKeybind } from "./config/keybind"

export const LEADER_TOKEN = "leader"
export const TALON_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const TALON_MODE_KEY = "talon.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(TALON_MODE_KEY, TALON_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(TALON_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(TALON_MODE_KEY, stack.at(-1)?.mode ?? TALON_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? TALON_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(TALON_MODE_KEY, undefined)
