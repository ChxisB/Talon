import { render, TimeToFirstDraw, useRenderer, useTerminalDimensions } from "@tui/solid"
...
import { createCliRenderer, MouseButton, type CliRenderer } from "@tui/core"
      ...
      const renderer = yield* Effect.acquireRelease(
        Effect.tryPromise(() =>
          createCliRenderer({
            ...
            useMouse: !Flag.TALON_DISABLE_MOUSE && input.config.mouse,
            consoleOptions: {
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.TALON_ROUTE ? JSON.parse(process.env.TALON_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.TALON_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.TALON_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.TALON_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("Talon")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.TALON_EXPERIMENTAL_WORKSPACES,
        ...
        : []),
      {
        ...
        suggested: true,
        slashName: "settings",
        slashAliases: ["preferences", "config"],
        run: () => {
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.TALON_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.TALON_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
