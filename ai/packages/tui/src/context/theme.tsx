import { CliRenderEvents, SyntaxStyle, type TerminalColors } from "@tui/core"
import { useRenderer } from "@tui/solid"
import {
  DEFAULT_THEMES,
  addTheme,
  allThemes,
...
const themeSource: ThemeSource = {
  async discover() {
    ...
    for (let current = process.cwd(); ; current = path.dirname(current)) {
      directories.push(path.join(current, ".talon"))
  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "talon")
        draft.active = typeof active === "string" ? active : "talon"
        draft.ready = false
      }),
    )
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "talon")
            return
          }
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.talon, store.mode)
    })
