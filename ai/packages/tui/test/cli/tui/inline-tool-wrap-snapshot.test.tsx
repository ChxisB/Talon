...
import type { ScrollBoxRenderable } from "@tui/core"
import { testRender, type JSX } from "@tui/solid"
import {
...
const tools: readonly ToolFixture[] = [
  {
    icon: "✱",
    label:
      'Grep "OPENCODE.*DB|database|sqlite|drizzle|dev.*db|data.*dir|xdg|APPDATA" in packages/talon/src (151 matches)',
  },
  {
    icon: "✱",
    label: 'Glob "**/*db*" in packages/talon (6 matches)',
  },
  {
    icon: "→",
    label: "Read packages/talon/src/storage/db.ts [offset=1, limit=130]",
  },
  {
    icon: "→",
    label: "Read packages/talon/src/index.ts [offset=1, limit=100]",
    error: "No LSP server available for this file type.",
  },
  {
    icon: "✱",
    label:
      'Grep "export const TALON_DB|TALON_DB|TALON_DEV|Global\\.Path\\.data|data =" in packages/talon/src (115 matches)',
  },
] as const
