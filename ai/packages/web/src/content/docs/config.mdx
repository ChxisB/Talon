...
1. **Remote config** (from `.well-known/talon`) - organizational defaults
2. **Global config** (`~/.config/talon/talon.json`) - user preferences
3. **Custom config** (`TALON_CONFIG` env var) - custom overrides
4. **Project config** (`talon.json` in project) - project-specific settings
5. **`.talon` directories** - agents, commands, plugins
6. **Inline config** (`TALON_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/talon/` on macOS) - admin-controlled
...
:::note
The `.talon` and `~/.config/talon` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/talon` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global OpenCode config in `~/.config/talon/talon.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/talon/tui.json`.
...
Add `talon.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
Specify a custom config file path using the `TALON_CONFIG` environment variable.
...
Specify a custom config directory using the `TALON_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.talon` directory, and should
...
The custom directory is loaded after the global config and `.talon` directories, so it **can override** their settings.
...
Drop an `talon.json` or `talon.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/talon/` |
| Linux    | `/etc/talon/`                         |
| Windows  | `%ProgramData%\talon`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, OpenCode reads managed preferences from the `ai.talon.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.talon.managed.plist`
2. `/Library/Managed Preferences/ai.talon.managed.plist`

The plist keys map directly to `talon.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.talon.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.talon.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.talon.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.talon</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`talon.ai/config.json`**](https://talon.ai/config.json).

TUI config uses [**`talon.ai/tui.json`**](https://talon.ai/tui.json).
...
Use `TALON_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `talon.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `talon serve` and `talon web` commands through the `server` option.
...
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `talon.local`. Useful for running multiple instances on the same network.
...
You can also define agents using markdown files in `~/.config/talon/agents/` or `.talon/agents/`. [Learn more here](/docs/agents).
...
This setting applies across all interfaces: TUI, CLI (`talon run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/talon/commands/` or `.talon/commands/`. [Learn more here](/docs/commands).
...
By default, talon **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
Place plugin files in `.talon/plugins/` or `~/.config/talon/plugins/`. You can also load plugins from npm through the `plugin` option.
