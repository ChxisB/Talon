...
talon søker etter disse stedene:

- Prosjektkonfigurasjon: `.talon/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/talon/skills/<name>/SKILL.md`
...
For prosjektlokale stier går talon opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.talon/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/talon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.talon/skills/git-release/SKILL.md` slik:
...
talon viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `talon.json`:
...
**For innebygde agenter** (i `talon.json`):
