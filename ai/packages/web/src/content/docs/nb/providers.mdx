...
1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Zen` og gå til [talon.ai/auth](https://talon.ai/zen).
...
1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Go`, og gå til [talon.ai/auth](https://talon.ai/zen).
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `talon.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i talon støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere talon til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
`talon.json` fil. Det anbefales også å deaktivere øktdeling.
...
4. Opprett eller oppdater `talon.json`-filen i prosjektkatalogen:
