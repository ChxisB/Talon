...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\talon\log`

Loggfiler navngis med tidsstempler (f.eks. `2025-01-09T123456.log`) og de siste 10 loggfilene beholdes.

Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `talon --log-level DEBUG`.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop kjører en lokal OpenCode-server (`talon-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.
...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/talon/talon.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\talon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/talon`
- **Linux**: slett `~/.cache/talon` (eller kjør `rm -rf ~/.cache/talon`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\talon`
...
Hvis `talon.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.

#### Sjekk miljøvariabler

Hvis du har `TALON_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `TALON_PORT` (eller velg en ledig port) og start på nytt.
...
- `talon.settings.dat` (stasjonær standardserver URL)
- `talon.global.dat` og `talon.workspace.*.dat` (UI tilstand som nylige servere/prosjekter)
...
1. Tøm leverandørens pakkebuffer:
   ...
   På Windows, trykk `WIN+R` og slett: `%USERPROFILE%\.cache\talon`
