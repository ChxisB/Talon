...
:::note
Dette verktøyet er bare tilgjengelig når `TALON_EXPERIMENTAL_LSP_TOOL=true` (eller `TALON_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker OpenCode-leverandøren eller når miljøvariabelen `TALON_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).
