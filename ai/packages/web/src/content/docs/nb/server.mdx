...
Kommandoen `talon serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `talon.local` |
...
Sett `TALON_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `talon` som standard, eller sett `TALON_SERVER_USERNAME` for å overstyre det. Dette gjelder både `talon serve` og `talon web`.
...
Når du kjører `talon` starter den en TUI og en server. Der TUI er
...
Du kan kjøre `talon serve` for å starte en frittstående server. Hvis du har
OpenCode TUI kjører, vil `talon serve` starte en ny server.
...
talon-serveren viser følgende APIer.
