...
For å beskytte tilgang, angi et passord ved hjelp av miljøvariabelen `TALON_SERVER_PASSWORD`:

```bash
TALON_SERVER_PASSWORD=secret talon web
```

Brukernavnet er satt til `talon` som standard, men kan endres med `TALON_SERVER_USERNAME`.
...
Du kan også konfigurere serverinnstillinger i `talon.json` konfigurasjonsfilen:
