...
Du kan definere MCP-servere i [OpenCode Config](https://talon.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/talon`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Hvis serveren krever autentisering, vil OpenCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `talon mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenCode lagre tokens sikkert i `~/.local/share/talon/mcp-auth.json`.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [talon-helicone-session](https://github.com/H2Shami/talon-helicone-session) plugin for automatisk å logge hver OpenCode-samtale som en økt i Helicone.

```bash
npm install -g talon-helicone-session
```
