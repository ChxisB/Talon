---
title: Konfigurasjon
description: Ved å bruke talon JSON-konfigurasjonen.
---
...
For eksempel, hvis din globale konfigurasjon setter `theme: "talon"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/talon`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/talon/talon.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`TALON_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`talon.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.talon` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`TALON_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.talon` og `~/.config/talon` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/talon`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/talon/talon.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `talon.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `TALON_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `TALON_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.talon` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.talon`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`talon.ai/config.json`**](https://talon.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `talon serve` og `talon web` gjennom alternativet `server`.
...
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `talon.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/talon/agents/` eller `.talon/agents/`. [Les mer her](/docs/agents).
...
Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`talon run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/talon/commands/` eller `.talon/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.talon/plugins/` eller `~/.config/talon/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
