...
in `~/.local/share/talon/auth.json`.
...
1. Run the `/connect` command in the TUI, select `OpenCode Zen`, and head to [talon.ai/auth](https://talon.ai/zen).
...
1. Run the `/connect` command in the TUI, select `OpenCode Go`, and head to [talon.ai/auth](https://talon.ai/zen).
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `talon.json`:
...
You can configure talon to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for talon to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your talon config.
...
`talon.json` file. It is also recommended to disable session sharing.
...
In the event you see a feature or model from Helicone that isn't configured automatically through talon, you can always configure it yourself.
...
You can configure talon to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure talon to use local models through LM Studio.
...
You can configure talon to use local models through Ollama.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your talon config.
...
5. You can also customize them through your talon config. Here's an example of specifying a provider
...
The model catalog is provided automatically. A minimal `talon.json` is all that's needed:
...
You can also customize models through your talon config. Here's an example of specifying provider routing order.
...
4. Create or update your `talon.json` file in your project directory:
...
2. For custom providers, check the talon config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your talon config.
