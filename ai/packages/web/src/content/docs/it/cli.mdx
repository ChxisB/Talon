...
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `TALON_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `TALON_SERVER_USERNAME` o `talon`) |
...
OpenCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `talon auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/talon/auth.json`.
...
Esegui talon in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `talon serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server talon in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `TALON_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `TALON_SERVER_USERNAME` o `talon`) |
...
Avvia un server HTTP che espone accesso API alle funzionalità di talon senza la TUI. Imposta `TALON_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `talon`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `TALON_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `talon`).
...
Aggiorna talon all'ultima versione o a una versione specifica.
...
La CLI di talon accetta i seguenti flag globali.
...
| ------------------------------------- | ------- | ----------------------------------------------------------- |
| `TALON_AUTO_SHARE`                 | boolean | Condivide automaticamente le sessioni                       |
| `TALON_GIT_BASH_PATH`              | string  | Percorso all'eseguibile Git Bash su Windows                 |
| `TALON_CONFIG`                     | string  | Percorso al file di configurazione                          |
| `TALON_TUI_CONFIG`                 | string  | Percorso al file di configurazione TUI                      |
| `TALON_CONFIG_DIR`                 | string  | Percorso alla directory di configurazione                   |
| `TALON_CONFIG_CONTENT`             | string  | Contenuto JSON di config inline                             |
| `TALON_DISABLE_AUTOUPDATE`         | boolean | Disabilita i controlli automatici di aggiornamento          |
| `TALON_DISABLE_PRUNE`              | boolean | Disabilita la potatura dei dati vecchi                      |
| `TALON_DISABLE_TERMINAL_TITLE`     | boolean | Disabilita aggiornamenti automatici del titolo terminale    |
| `TALON_PERMISSION`                 | string  | Config permessi JSON inline                                 |
| `TALON_DISABLE_DEFAULT_PLUGINS`    | boolean | Disabilita i plugin di default                              |
| `TALON_DISABLE_LSP_DOWNLOAD`       | boolean | Disabilita download automatico dei server LSP               |
| `TALON_ENABLE_EXPERIMENTAL_MODELS` | boolean | Abilita modelli sperimentali                                |
| `TALON_DISABLE_AUTOCOMPACT`        | boolean | Disabilita compaction automatica del contesto               |
| `TALON_DISABLE_CLAUDE_CODE`        | boolean | Disabilita lettura da `.claude` (prompt + skill)            |
| `TALON_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disabilita lettura di `~/.claude/CLAUDE.md`                 |
| `TALON_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disabilita caricamento di `.claude/skills`                  |
| `TALON_DISABLE_MODELS_FETCH`       | boolean | Disabilita fetch dei modelli da fonti remote                |
| `TALON_FAKE_VCS`                   | string  | Provider VCS finto per scopi di test                        |
| `TALON_CLIENT`                     | string  | Identificatore client (default `cli`)                       |
| `TALON_ENABLE_EXA`                 | boolean | Abilita gli strumenti di web search Exa                     |
| `TALON_SERVER_PASSWORD`            | string  | Abilita basic auth per `serve`/`web`                        |
| `TALON_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `talon`)     |
| `TALON_MODELS_URL`                 | string  | URL personalizzato per recuperare la configurazione modelli |
...
| Variabile                                       | Tipo    | Descrizione                                             |
| ----------------------------------------------- | ------- | ------------------------------------------------------- |
| `TALON_EXPERIMENTAL`                         | boolean | Abilita funzionalità sperimentali sotto il flag globale |
| `TALON_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Abilita icon discovery                                  |
| `TALON_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disabilita copy on select nella TUI                     |
| `TALON_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Timeout di default per comandi bash in ms               |
| `TALON_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Massimo token di output per risposte LLM                |
| `TALON_EXPERIMENTAL_FILEWATCHER`             | boolean | Abilita file watcher per l'intera dir                   |
| `TALON_EXPERIMENTAL_OXFMT`                   | boolean | Abilita formatter oxfmt                                 |
| `TALON_EXPERIMENTAL_LSP_TOOL`                | boolean | Abilita strumento LSP sperimentale                      |
| `TALON_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disabilita file watcher                                 |
| `TALON_EXPERIMENTAL_EXA`                     | boolean | Abilita funzionalità Exa sperimentali                   |
| `TALON_EXPERIMENTAL_LSP_TY`                  | boolean | Abilita TY LSP per i file python                        |
| `TALON_EXPERIMENTAL_PLAN_MODE`               | boolean | Abilita plan mode                                       |
| `TALON_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Abilita task subagent in background                     |
| `TALON_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Abilita sistema eventi sperimentale                     |
| `TALON_EXPERIMENTAL_NATIVE_LLM`              | boolean | Abilita percorso nativo di richiesta LLM                |
| `TALON_EXPERIMENTAL_PARALLEL`                | boolean | Abilita esecuzione parallela della ricerca web          |
| `TALON_EXPERIMENTAL_SCOUT`                   | boolean | Abilita subagent Scout                                  |
| `TALON_EXPERIMENTAL_WORKSPACES`              | boolean | Abilita supporto workspace                              |
