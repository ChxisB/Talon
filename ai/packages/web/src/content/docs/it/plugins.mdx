...
- `.talon/plugins/` - plugin a livello progetto
- `~/.config/talon/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/talon/node_modules/`.
...
1. Config globale (`~/.config/talon/talon.json`)
2. Config di progetto (`talon.json`)
3. Directory plugin globale (`~/.config/talon/plugins/`)
4. Directory plugin di progetto (`.talon/plugins/`)
...
- `client`: un client SDK di talon per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere talon.
...
Impedisci a talon di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a talon:
...
L'helper `tool` crea uno strumento personalizzato che talon può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in talon insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://talon.ai/docs/sdk) per i dettagli.
