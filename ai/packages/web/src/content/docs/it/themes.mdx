...
Per impostazione predefinita, OpenCode usa il tema `talon`.
...
2. **Directory di configurazione utente** - in `~/.config/talon/themes/*.json` o `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.talon/themes/*.json`
4. **Directory di lavoro corrente** - in `./.talon/themes/*.json`
