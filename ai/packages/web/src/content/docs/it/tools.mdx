...
:::note
Questo strumento e' disponibile solo quando `TALON_EXPERIMENTAL_LSP_TOOL=true` (oppure `TALON_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider OpenCode o quando la variabile d'ambiente `TALON_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).
