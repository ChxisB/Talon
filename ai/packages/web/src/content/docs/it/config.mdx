...
1. **Config remota** (da `.well-known/talon`) - default dell'organizzazione
2. **Config globale** (`~/.config/talon/talon.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `TALON_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`talon.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.talon`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `TALON_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.talon` e `~/.config/talon` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/talon`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di OpenCode in `~/.config/talon/talon.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/talon/tui.json`.
...
Aggiungi `talon.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `TALON_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `TALON_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.talon` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.talon`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`talon.ai/config.json`**](https://talon.ai/config.json).

La configurazione TUI usa [**`talon.ai/tui.json`**](https://talon.ai/tui.json).
...
Usa `TALON_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `talon.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `talon serve` e `talon web` tramite l'opzione `server`.
...
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `talon.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi anche definire agenti usando file markdown in `~/.config/talon/agents/` o `.talon/agents/`. [Scopri di piu](/docs/agents).
...
Questa impostazione si applica a tutte le interfacce: TUI, CLI (`talon run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/talon/commands/` o `.talon/commands/`. [Scopri di piu](/docs/commands).
...
Per impostazione predefinita, talon **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
Metti i file dei plugin in `.talon/plugins/` o `~/.config/talon/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
