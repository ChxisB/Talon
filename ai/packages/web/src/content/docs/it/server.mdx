---
title: Server
description: Interagisci con il server talon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `talon serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client talon.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `talon.local` |
...
Imposta `TALON_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `talon`, oppure imposta `TALON_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `talon serve` sia per `talon web`.
...
Quando esegui `talon` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server talon per interagire con talon in modo programmatico.
:::

Questa architettura permette a talon di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `talon serve` per avviare un server standalone. Se la TUI di talon e gia in esecuzione, `talon serve` avviera un nuovo server.
...
Il server talon espone le seguenti API.
