...
Questo è separato da `talon.json`, che configura il comportamento di server/runtime.
...
Usa `TALON_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
...
- `sound_pack` - ID del sound pack da usare. Default: `talon.default`.
