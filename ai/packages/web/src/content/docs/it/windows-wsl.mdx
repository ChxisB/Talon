...
:::caution
Quando usi `--hostname 0.0.0.0`, imposta `TALON_SERVER_PASSWORD` per proteggere il server.
...
1. **Esegui `talon web` nel terminale WSL** invece che in PowerShell:
...
Eseguire `talon web` da WSL garantisce accesso corretto al file system e integrazione del terminale, restando comunque accessibile dal browser Windows.
...
- Configurazione e sessioni di OpenCode sono salvate nell'ambiente WSL in `~/.local/share/talon/`
