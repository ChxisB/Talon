...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\talon\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `talon --log-level DEBUG`.

---

## Archiviazione

talon salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop esegue in background un server locale di OpenCode (il sidecar `talon-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/talon/talon.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\talon\plugins`
...
- **Linux**: elimina `~/.cache/talon` (oppure esegui `rm -rf ~/.cache/talon`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\talon`
...
Se il tuo `talon.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `TALON_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `TALON_PORT` (o scegli una porta libera) e riavvia.
...
- `talon.settings.dat` (desktop default server URL)
- `talon.global.dat` e `talon.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
3. Assicurati di avere l'ultima versione con `talon upgrade`
...
Per capire a quali modelli hai accesso, esegui `talon models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\talon`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. talon installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\talon`

2. Riavvia talon per reinstallare i pacchetti provider piu recenti

Questo forzera talon a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
talon rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
