...
1. Esegui `/connect` nella TUI, seleziona talon e vai su [talon.ai/auth](https://talon.ai/auth).
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `talon.json`:
...
Puoi configurare talon per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché talon funzioni correttamente.
   :::
...
file `talon.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `talon.json`:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da talon, puoi sempre configurarlo tu stesso.
...
Puoi configurare talon per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare talon per usare modelli locali tramite LM Studio.
...
Puoi configurare talon per usare modelli locali tramite Ollama.
...
4. Crea o aggiorna il tuo file `talon.json` nella directory del progetto:
...
2. Per provider personalizzati, controlla la config di talon e:
