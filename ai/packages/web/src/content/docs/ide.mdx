...
OpenCode integrates with VS Code, Cursor, or any IDE that supports a terminal. Just run `talon` in the terminal to get started.
...
3. Run `talon` - the extension installs automatically
...
- Ensure you’re running `talon` in the integrated terminal.
