...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\talon\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `talon --log-level DEBUG`.

---

## Lagring

talon gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop kører en lokal OpenCode-server (`talon-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (eller `~/.config/talon/talon.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/talon/talon.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\talon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/talon`
- **Linux**: slet `~/.cache/talon` (eller kør `rm -rf ~/.cache/talon`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\talon`
...
Hvis `talon.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `TALON_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `TALON_PORT` (eller vælg en ledig port) og start på ny.
...
- `talon.settings.dat` (skrivebordsstandardserver URL)
- `talon.global.dat` og `talon.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
3. Sørg for at du har den nyeste version med `talon upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `talon models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\talon`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. talon installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\talon`

2. Start talon på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge talon til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
talon vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
