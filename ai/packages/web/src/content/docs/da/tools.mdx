...
:::note
Dette verktøyet er bare tilgængelig når `TALON_EXPERIMENTAL_LSP_TOOL=true` (eller `TALON_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `TALON_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
