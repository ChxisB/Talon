...
- `.talon/plugins/` - Plugins på projektniveau
- `~/.config/talon/plugins/` - Globale plugins
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/talon/node_modules/`.
...
1. Global konfiguration (`~/.config/talon/talon.json`)
2. Projektkonfiguration (`talon.json`)
3. Global plugin-mappe (`~/.config/talon/plugins/`)
4. Projekt plugin bibliotek (`.talon/plugins/`)
...
- `client`: En talon SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide talon.
...
Undgå talon i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til talon:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som talon kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for talon sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://talon.ai/docs/sdk) for detaljer.
