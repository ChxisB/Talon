...
Dette er adskilt fra `talon.json`, som konfigurerer server-/kørselstidsadfærd.
...
Brug `TALON_TUI_CONFIG` til at indlæse en brugerdefineret TUI-konfigurationssti.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `talon.default`.
