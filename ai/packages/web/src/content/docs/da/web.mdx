...
:::caution
Hvis `TALON_SERVER_PASSWORD` ikke er indstillet, vil serveren være usikret. Dette er fint til lokal brug, men bør indstilles til netværksadgang.
:::

:::tip[Windows-brugere]
For den bedste oplevelse skal du køre `talon web` fra [WSL](/docs/windows-wsl) i stedet for PowerShell. Dette sikrer korrekt filsystemadgang og terminalintegration.
:::
...
Dette indstiller automatisk værtsnavnet til `0.0.0.0` og annoncerer serveren som `talon.local`.
...
For at beskytte adgangen skal du angive en adgangskode ved hjælp af `TALON_SERVER_PASSWORD` miljøvariablen:

```bash
TALON_SERVER_PASSWORD=secret talon web
```

Brugernavnet er som standard `talon`, men kan ændres med `TALON_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `talon.json`-konfigurationsfil:
