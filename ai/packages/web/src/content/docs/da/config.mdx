...
1. **Fjernkonfiguration** (fra `.well-known/talon`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/talon/talon.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`TALON_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`talon.json` i projekt) - projektspecifikke indstillinger
5. **`.talon` mapper** - agent, kommandoer, plugins
6. **Inline config** (`TALON_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.talon` og `~/.config/talon` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/talon`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenCode-konfiguration i `~/.config/talon/talon.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `talon.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `TALON_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `TALON_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.talon` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.talon` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`talon.ai/config.json`**](https://talon.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `talon serve` og `talon web` gennem indstillingen `server`.
...
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `talon.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/talon/agents/` eller `.talon/agents/`. [Learn more here](/docs/agents).
...
Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`talon run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/talon/commands/` eller `.talon/commands/`. [Learn more here](/docs/commands).
...
Som standard talon **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
Placer plugin-filer i `.talon/plugins/` eller `~/.config/talon/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
