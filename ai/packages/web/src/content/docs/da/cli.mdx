...
| <nobr><code>{"--session"}</code></nobr>  | `-s` | Sessions-id for at fortsætte                                                      |
| <nobr><code>{"--fork"}</code></nobr>     |      | Forgren sessionen ved fortsættelse (brug med `--continue` eller `--session`)      |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `TALON_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `TALON_SERVER_USERNAME` eller `talon`) |
...
OpenCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `talon auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/talon/auth.json`.
...
Kør talon i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `talon serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende talon-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `TALON_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `TALON_SERVER_USERNAME` eller `talon`)   |
...
Dette starter en HTTP-server, der giver API-adgang til talon-funktionalitet uden TUI-grænsefladen. Indstil `TALON_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `talon`).
...
Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenCode via en webgrænseflade. Indstil `TALON_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `talon`).
...
talon CLI tager følgende globale flag.
...
| ------------------------------------- | ------- | --------------------------------------------------------------------- |
| `TALON_AUTO_SHARE`                 | boolean | Del automatisk session                                                |
| `TALON_GIT_BASH_PATH`              | string  | Sti til Git Bash eksekverbar på Windows                               |
| `TALON_CONFIG`                     | string  | Sti til konfigurationsfil                                             |
| `TALON_TUI_CONFIG`                 | string  | Sti til TUI-konfigurationsfil                                         |
| `TALON_CONFIG_DIR`                 | string  | Sti til konfigurationsmappe                                           |
| `TALON_CONFIG_CONTENT`             | string  | Indbygget json-konfigurationsindhold                                  |
| `TALON_DISABLE_AUTOUPDATE`         | boolean | Deaktiver automatisk opdateringskontrol                               |
| `TALON_DISABLE_PRUNE`              | boolean | Deaktiver beskæring af gamle data                                     |
| `TALON_DISABLE_TERMINAL_TITLE`     | boolean | Deaktiver automatisk opdatering af terminaltitel                      |
| `TALON_PERMISSION`                 | string  | Indbygget json-tilladelseskonfiguration                               |
| `TALON_DISABLE_DEFAULT_PLUGINS`    | boolean | Deaktiver standard plugins                                            |
| `TALON_DISABLE_LSP_DOWNLOAD`       | boolean | Deaktiver automatisk LSP-serverdownloads                              |
| `TALON_ENABLE_EXPERIMENTAL_MODELS` | boolean | Aktive eksperimentelle modeller                                       |
| `TALON_DISABLE_AUTOCOMPACT`        | boolean | Deaktiver automatisk kontekstkomprimering                             |
| `TALON_DISABLE_CLAUDE_CODE`        | boolean | Deaktiver læsning fra `.claude` (prompt + færdigheder)                |
| `TALON_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Deaktiver læsning `~/.claude/CLAUDE.md`                               |
| `TALON_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Deaktiver indlæsning af `.claude/skills`                              |
| `TALON_DISABLE_MODELS_FETCH`       | boolean | Deaktivering af modeller fra eksterne kilder                          |
| `TALON_FAKE_VCS`                   | string  | Falsk VCS-udbyder til testformål                                      |
| `TALON_CLIENT`                     | string  | Klient-id (standard til `cli`)                                        |
| `TALON_ENABLE_EXA`                 | boolean | Aktiver Exa-websøgeværktøjer                                          |
| `TALON_SERVER_PASSWORD`            | string  | Aktiver grundlæggende godkendelse for `serve`/`web`                   |
| `TALON_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `talon`) |
| `TALON_MODELS_URL`                 | string  | Brugerdefineret URL til hentning af modelkonfiguration                |
...
| Variabel                                        | Skriv   | Beskrivelse                                          |
| ----------------------------------------------- | ------- | ---------------------------------------------------- |
| `TALON_EXPERIMENTAL`                         | boolean | Aktiver eksperimentelle funktioner under samleflaget |
| `TALON_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Aktiver ikonopdagelse                                |
| `TALON_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Deaktiver kopi ved valg i TUI                        |
| `TALON_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | nummer  | Standard timeout for bash-kommandoer i ms            |
| `TALON_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | nummer  | Maks. output-tokens for LLM-svar                     |
| `TALON_EXPERIMENTAL_FILEWATCHER`             | boolean | Aktiver filovervågning for hele dir                  |
| `TALON_EXPERIMENTAL_OXFMT`                   | boolean | Aktiver oxfmt formatter                              |
| `TALON_EXPERIMENTAL_LSP_TOOL`                | boolean | Aktive eksperimenter LSP værktøj                     |
| `TALON_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Deaktiver filovervågning                             |
| `TALON_EXPERIMENTAL_EXA`                     | boolean | Aktive eksperimenter Exa-funktioner                  |
| `TALON_EXPERIMENTAL_LSP_TY`                  | boolean | Aktiver TY LSP for python-filer                      |
| `TALON_EXPERIMENTAL_PLAN_MODE`               | boolean | Aktiver plantilstand                                 |
| `TALON_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Aktiver baggrundsopgaver for subagenter              |
| `TALON_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Aktiver eksperimentelt hændelsessystem               |
| `TALON_EXPERIMENTAL_NATIVE_LLM`              | boolean | Aktiver native LLM-anmodningssti                     |
| `TALON_EXPERIMENTAL_PARALLEL`                | boolean | Aktiver parallel udførelse af websøgning             |
| `TALON_EXPERIMENTAL_SCOUT`                   | boolean | Aktiver Scout-subagent                               |
| `TALON_EXPERIMENTAL_WORKSPACES`              | boolean | Aktiver workspace-understøttelse                     |
