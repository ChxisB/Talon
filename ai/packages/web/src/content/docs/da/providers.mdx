...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/talon/auth.json`.
...
1. Kør kommandoen `/connect` i TUI, vælg talon og gå til [talon.ai/auth](https://talon.ai/auth).
...
1. Kør kommandoen `/connect` i TUI, vælg `OpenCode Go`, og gå til [talon.ai/auth](https://talon.ai/zen).
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `talon.json`:
...
Du kan konfigurere talon til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at talon skal fungere korrekt.
   :::
...
gemme tokenet i talon auth-lagring.
:::
...
`talon.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem talon, kan du altid konfigurere den selv.
...
Du kan konfigurere talon til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere talon til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere talon til at bruge lokale modeller gennem Ollama.
...
4. Opret eller opdater `talon.json`-filen i projektkataloget:
