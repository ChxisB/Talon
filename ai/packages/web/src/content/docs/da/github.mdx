...
OpenCode integreres med din GitHub arbejdsgang. Nævn `/talon` eller `/oc` i din kommentar, og OpenCode vil udføre opgaver i din GitHub Actions-løber.
...
1. **Installationsprogrammet GitHub-appen**
   ...
   Gå over til [**github.com/apps/talon-agent**](https://github.com/apps/talon-agent). Sørg for, at det er installeret på mållageret.

2. **Tilføj arbejdsgangen**

   Tilføj følgende workflow-fil til `.github/workflows/talon.yml` i din repo. Sørg for at indstille de relevante `model` og nødvendige API-nøgler i `env`.
...
| ----------------------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentarer og problemer eller PR       | Nævn `/talon` eller `/oc` i din kommentar. OpenCode læser kontekst og kan oprette brancher, åbne PR'er eller svare.  |
| `pull_request_review_comment` | Kommenter specifikke kodelinjer i en PR | Nævn `/talon` eller `/oc`, mens du gennemgår koden. OpenCode modtager filsti, linjenumre og diff-kontekst.           |
