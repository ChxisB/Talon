---
title: Server
description: Interager med talon-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `talon serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en talon-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `talon.local` |
...
Indstil `TALON_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `talon`, eller indstil `TALON_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `talon serve` og `talon web`.
...
Når du kører `talon` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug talon-serveren til at interagere med talon programmatisk.
:::

Denne arkitektur lader talon understøtte flere klienter og lader dig interagere med talon programmatisk.

Du kan køre `talon serve` for at starte en selvstændig server. Hvis du har
talon TUI kørende, vil `talon serve` starte en ny server.
