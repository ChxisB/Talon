---
title: Regler
description: Angiv tilpassede instruktioner til talon.
---
...
For at lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i talon.

:::tip
Du bør overgi prosjektets `AGENTS.md`-fil til Git.
:::

Dette vil skanne prosjektet og alt dets innhold for at forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper talon at navigere i prosjektet bedre.
...
talon støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/talon/AGENTS.md`-fil. Dette blir brugt på alle talon-sessioner.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (bruges hvis ingen `~/.config/talon/AGENTS.md` eksisterer)
...
Når talon starter, ser den etter regelfiler i denne rekkefølgen:

1. **Lokale filer** ved at gå opp fra nuværende katalog (`AGENTS.md`, `CLAUDE.md`)
2. **Global fil** på `~/.config/talon/AGENTS.md`
3. **Claude Code-fil** på `~/.claude/CLAUDE.md` (med mindre de er deaktivert)

Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, bruges bare `AGENTS.md`. På samme måte har `~/.config/talon/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Brugerdefinerede instruktioner

Du kan spesifisere egendefinerte instruksjonsfiler i din `talon.json` eller den globale `~/.config/talon/talon.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
Selv om talon ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:

### Brug af talon.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `talon.json`:
...
Du kan lære talon at lese eksterne filer ved at gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at talon laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er brug av `talon.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
