...
:::caution
Når du bruger `--hostname 0.0.0.0`, skal du sætte `TALON_SERVER_PASSWORD` for at sikre serveren.
...
1. **Kør `talon web` i WSL-terminalen** i stedet for PowerShell:
...
Når du kører `talon web` fra WSL, får du korrekt filsystemadgang og terminalintegration, samtidig med at det stadig er tilgængeligt fra din Windows-browser.
...
- Din OpenCode-konfiguration og dine sessioner gemmes i WSL-miljøet i `~/.local/share/talon/`
