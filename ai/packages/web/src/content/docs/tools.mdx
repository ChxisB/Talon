...
:::note
This tool is only available when `TALON_EXPERIMENTAL_LSP_TOOL=true` (or `TALON_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the OpenCode provider or when the `TALON_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).
