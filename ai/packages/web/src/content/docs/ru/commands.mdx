...
Создайте `.talon/commands/test.md`:

```md title=".talon/commands/test.md"
---
...
Вы можете добавлять собственные команды через конфигурацию talon или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем talon [config](/docs/config):

```json title="talon.jsonc" {4-12}
{
...
- Глобальный: `~/.config/talon/commands/`
- Для каждого проекта: `.talon/commands/`

```markdown title="~/.config/talon/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".talon/commands/component.md"
---
...
```md title=".talon/commands/create-file.md"
---
...
```md title=".talon/commands/analyze-coverage.md"
---
...
```md title=".talon/commands/review-changes.md"
---
...
```md title=".talon/commands/review-component.md"
---
...
talon включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
