...
С помощью talon вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию talon использует нашу собственную тему `talon`.
...
talon поставляется с несколькими встроенными темами.
...
- Хотите, чтобы talon соответствовал внешнему виду их терминала
...
talon поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/talon/themes/*.json` или `$XDG_CONFIG_HOME/talon/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.talon/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.talon/themes/*.json`.
...
```bash no-frame
mkdir -p ~/.config/talon/themes
vim ~/.config/talon/themes/my-theme.json
```

Для тем проекта:

```bash no-frame
mkdir -p .talon/themes
vim .talon/themes/my-theme.json
```
...
{
  "$schema": "https://talon.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
