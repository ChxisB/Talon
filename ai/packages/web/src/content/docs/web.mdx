...
:::caution
If `TALON_SERVER_PASSWORD` is not set, the server will be unsecured. This is fine for local use but should be set for network access.
:::

:::tip[Windows Users]
For the best experience, run `talon web` from [WSL](/docs/windows-wsl) rather than PowerShell. This ensures proper file system access and terminal integration.
:::
...
This automatically sets the hostname to `0.0.0.0` and advertises the server as `talon.local`.
...
To protect access, set a password using the `TALON_SERVER_PASSWORD` environment variable:

```bash
TALON_SERVER_PASSWORD=secret talon web
```

The username defaults to `talon` but can be changed with `TALON_SERVER_USERNAME`.
...
You can also configure server settings in your `talon.json` config file:
