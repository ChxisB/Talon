...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\talon\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `talon --log-level DEBUG`.

---

## Storage

talon stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop runs a local OpenCode server (the `talon-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (or `~/.config/talon/talon.json`)
- **macOS/Linux** (older installs): `~/.local/share/talon/talon.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Global plugins**
  ...
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\talon\plugins`
...
- **Linux**: delete `~/.cache/talon` (or run `rm -rf ~/.cache/talon`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\talon`
...
If your `talon.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `TALON_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `TALON_PORT` (or pick a free port) and restart.
...
- `talon.global.dat` and `talon.workspace.*.dat` (UI state like recent servers/projects)
...
3. Ensure you have the latest version with `talon upgrade`
...
To figure out what models you have access to, run `talon models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\talon`
...
If you encounter API call errors, this may be due to outdated provider packages. talon dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\talon`

2. Restart talon to reinstall the latest provider packages

This will force talon to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
talon will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
