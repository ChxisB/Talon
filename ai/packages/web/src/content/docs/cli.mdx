...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.talon/agent` based on the prompt)                                                                                                                         |
...
| <nobr><code>{"--session"}</code></nobr>  | `-s`  | Session ID to continue                                                     |
| <nobr><code>{"--fork"}</code></nobr>     |       | Fork the session when continuing (use with `--continue` or `--session`)    |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `TALON_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `TALON_SERVER_USERNAME` or `talon`) |
...
OpenCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `talon auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/talon/auth.json`.
...
Run talon in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `talon serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running talon server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `TALON_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `TALON_SERVER_USERNAME` or `talon`) |
...
This starts an HTTP server that provides API access to talon functionality without the TUI interface. Set `TALON_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `talon`).
...
```bash
talon session delete <sessionID>
```
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `TALON_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `talon`).
...
Updates talon to the latest version or a specific version.
...
The talon CLI takes the following global flags.
...
| ------------------------------------- | ------- | ------------------------------------------------- |
| `TALON_AUTO_SHARE`                 | boolean | Automatically share sessions                      |
| `TALON_GIT_BASH_PATH`              | string  | Path to Git Bash executable on Windows            |
| `TALON_CONFIG`                     | string  | Path to config file                               |
| `TALON_TUI_CONFIG`                 | string  | Path to TUI config file                           |
| `TALON_CONFIG_DIR`                 | string  | Path to config directory                          |
| `TALON_CONFIG_CONTENT`             | string  | Inline json config content                        |
| `TALON_DISABLE_AUTOUPDATE`         | boolean | Disable automatic update checks                   |
| `TALON_DISABLE_PRUNE`              | boolean | Disable pruning of old data                       |
| `TALON_DISABLE_TERMINAL_TITLE`     | boolean | Disable automatic terminal title updates          |
| `TALON_PERMISSION`                 | string  | Inlined json permissions config                   |
| `TALON_DISABLE_DEFAULT_PLUGINS`    | boolean | Disable default plugins                           |
| `TALON_DISABLE_LSP_DOWNLOAD`       | boolean | Disable automatic LSP server downloads            |
| `TALON_ENABLE_EXPERIMENTAL_MODELS` | boolean | Enable experimental models                        |
| `TALON_DISABLE_AUTOCOMPACT`        | boolean | Disable automatic context compaction              |
| `TALON_DISABLE_CLAUDE_CODE`        | boolean | Disable reading from `.claude` (prompt + skills)  |
| `TALON_DISABLE_CLAUDE_CODE_PROMPT` | boolean | Disable reading `~/.claude/CLAUDE.md`             |
| `TALON_DISABLE_CLAUDE_CODE_SKILLS` | boolean | Disable loading `.claude/skills`                  |
| `TALON_DISABLE_MODELS_FETCH`       | boolean | Disable fetching models from remote sources       |
| `TALON_DISABLE_MOUSE`              | boolean | Disable mouse capture in the TUI                  |
| `TALON_FAKE_VCS`                   | string  | Fake VCS provider for testing purposes            |
| `TALON_CLIENT`                     | string  | Client identifier (defaults to `cli`)             |
| `TALON_ENABLE_EXA`                 | boolean | Enable Exa web search tools                       |
| `TALON_SERVER_PASSWORD`            | string  | Enable basic auth for `serve`/`web`               |
| `TALON_SERVER_USERNAME`            | string  | Override basic auth username (default `talon`) |
| `TALON_MODELS_URL`                 | string  | Custom URL for fetching models configuration      |
...
| Variable                                        | Type    | Description                             |
| ----------------------------------------------- | ------- | --------------------------------------- |
| `TALON_EXPERIMENTAL`                         | boolean | Enable the experimental umbrella flag   |
| `TALON_EXPERIMENTAL_ICON_DISCOVERY`          | boolean | Enable icon discovery                   |
| `TALON_EXPERIMENTAL_DISABLE_COPY_ON_SELECT`  | boolean | Disable copy on select in TUI           |
| `TALON_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS` | number  | Default timeout for bash commands in ms |
| `TALON_EXPERIMENTAL_OUTPUT_TOKEN_MAX`        | number  | Max output tokens for LLM responses     |
| `TALON_EXPERIMENTAL_FILEWATCHER`             | boolean | Enable file watcher for entire dir      |
| `TALON_EXPERIMENTAL_OXFMT`                   | boolean | Enable oxfmt formatter                  |
| `TALON_EXPERIMENTAL_LSP_TOOL`                | boolean | Enable experimental LSP tool            |
| `TALON_EXPERIMENTAL_DISABLE_FILEWATCHER`     | boolean | Disable file watcher                    |
| `TALON_EXPERIMENTAL_EXA`                     | boolean | Enable experimental Exa features        |
| `TALON_EXPERIMENTAL_LSP_TY`                  | boolean | Enable TY LSP for python files          |
| `TALON_EXPERIMENTAL_PLAN_MODE`               | boolean | Enable plan mode                        |
| `TALON_EXPERIMENTAL_BACKGROUND_SUBAGENTS`    | boolean | Enable background subagent tasks        |
| `TALON_EXPERIMENTAL_EVENT_SYSTEM`            | boolean | Enable experimental event system        |
| `TALON_EXPERIMENTAL_NATIVE_LLM`              | boolean | Enable native LLM request path          |
| `TALON_EXPERIMENTAL_PARALLEL`                | boolean | Enable parallel web search execution    |
| `TALON_EXPERIMENTAL_SCOUT`                   | boolean | Enable Scout subagent                   |
| `TALON_EXPERIMENTAL_WORKSPACES`              | boolean | Enable workspace support                |
