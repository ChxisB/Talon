...
Aby debugować problemy z talon, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\talon\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `talon --log-level DEBUG`.

---

## Przechowywanie danych

talon przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\talon`
...
talon Desktop uruchamia w tle lokalny serwer talon (sidecar `talon-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (lub `~/.config/talon/talon.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/talon/talon.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\talon\talon.jsonc`
...
talon może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/talon/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\talon\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby talon mógł ją odbudować.

1. Całkowicie zamknij talon Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/talon`
- **Linux**: usuń `~/.cache/talon` (lub uruchom `rm -rf ~/.cache/talon`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\talon`

3. Uruchom ponownie talon Desktop.

---

### Napraw problemy z połączeniem z serwerem

talon Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `talon.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `TALON_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `TALON_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows talon Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami talon.

---

### Brak powiadomień

talon Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla talon w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie talon Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji talon Desktop):

- `talon.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `talon.global.dat` i `talon.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `talon upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `talon models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\talon`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. talon dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\talon`

2. Uruchom ponownie kod talon, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to talon do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
talon wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
