...
talon używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.
...
OpenCode Zen to lista modeli dostarczonych przez zespół talon, które zostały
...
1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Zen` i przejdź do [talon.ai/auth](https://talon.ai/zen).
...
Działa jak każdy inny dostawca w talon i jest całkowicie opcjonalny w użyciu.

---

## OpenCode Go

OpenCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół talon, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Go` i przejdź do [talon.ai/auth](https://talon.ai/zen).
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `talon.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w talon nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować talon do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod talon działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
talon używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
...
`talon.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą talon, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację talon jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja talon będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować talon tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować talon tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować talon tak, aby korzystał z modeli lokalnych poprzez Ollamę.
...
6. **Ważne**: Przed użyciem modeli chmurowych w talon musisz pobrać informacje o modelu lokalnie:
...
4. Utwórz lub zaktualizuj plik `talon.json` w katalogu projektu:
...
Pola `limit` pozwalają talon zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację talon i:
