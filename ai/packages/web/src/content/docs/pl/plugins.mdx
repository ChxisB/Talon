---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć talon.
---

Wtyczki umożliwiające wyłączenie talon poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania talon.
...
- `.talon/plugins/` - Wtyczki na poziomie projektu
- `~/.config/talon/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/talon/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/talon/talon.json`)
2. Konfiguracja projektu (`talon.json`)
3. Globalny katalog wtyczek (`~/.config/talon/plugins/`)
4. Katalog wtyczek projektu (`.talon/plugins/`)
...
talon uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z talon do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do talon.
...
:::note
Jeśli korzystasz z aplikacji komputerowej talon, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj talon czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do talon:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod talon. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://talon.ai/docs/sdk).
