...
Dzięki talon możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie talon używa naszego własnego motywu `talon`.
...
talon ma kilka wbudowanych motywów.
...
talon obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/talon/themes/*.json` lub `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.talon/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.talon/themes/*.json`
