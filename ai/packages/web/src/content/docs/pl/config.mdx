...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "talon"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/talon`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/talon/talon.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `TALON_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`talon.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.talon`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `TALON_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.talon` i `~/.config/talon` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/talon`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/talon/talon.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/talon/tui.json`.
...
Dodaj `talon.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `TALON_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `TALON_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.talon` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.talon`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`talon.ai/config.json`**](https://talon.ai/config.json).
...
Użyj `TALON_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `talon.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `talon serve` i `talon web` za pomocą opcji `server`.
...
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `talon.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/talon/agents/` lub `.talon/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Dotyczy to wszystkich interfejsów: TUI, CLI (`talon run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/talon/commands/` lub `.talon/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.talon/plugins/` lub `~/.config/talon/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
