...
Możesz dodać dodatkowe informacje do talon za pomocą _Model Context Protocol_, w skrócie MCP. talon obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [talon Config](https://talon.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/talon`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
talon automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, talon:
...
Jeśli serwer wymaga uwierzytelnienia, talon poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `talon mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji talon bezpieczne przechowa tokeny w `~/.local/share/talon/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w talon, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie talon, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć talon z kontem Sentry.
