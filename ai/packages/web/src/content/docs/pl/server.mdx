---
title: Serwer
description: Interakcja z serwerem talon za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `talon serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient talon.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `talon.local` |
...
Ustaw `TALON_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `talon` lub ustaw `TALON_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `talon serve`, jak i `talon web`.
...
Kiedy uruchomisz `talon`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera talon do programowej interakcji z talon.
:::

Ta architektura umożliwia obsługę wielu klientów przez talon i programową interakcję z kodem otwartym.

Możesz uruchomić `talon serve`, aby uruchomić samodzielny serwer. Jeśli masz
talon TUI działa, `talon serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki talon [IDE](/docs/ide).
...
Serwer talon udostępnia następujące interfejsy API.
