...
Umiejętności agenta pozwalają talon odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
talon przeszukuje te lokalizacje:

- Project config: `.talon/skills/<name>/SKILL.md`
- Global config: `~/.config/talon/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu talon przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.talon/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/talon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.talon/skills/git-release/SKILL.md` w ten sposób:
...
talon wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `talon.json`:
...
**Dla agentów wbudowanych** (w `talon.json`):
