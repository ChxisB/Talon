...
:::note
You can disable automatic LSP server downloads by setting the `TALON_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and talon opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your talon config.
