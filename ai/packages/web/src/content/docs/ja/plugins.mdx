...
- `.talon/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/talon/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/talon/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/talon/talon.json`)
2. プロジェクト設定 (`talon.json`)
3. グローバルプラグインディレクトリ (`~/.config/talon/plugins/`)
4. プロジェクトプラグインディレクトリ (`.talon/plugins/`)
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://talon.ai/docs/sdk) を参照してください。
