...
1. TUI で `/connect` コマンドを実行し、`OpenCode Zen` を選択して、[talon.ai/zen](https://talon.ai/zen) にアクセスします。
...
1. TUI で `/connect` コマンドを実行し、`OpenCode Go` を選択して、[talon.ai/zen](https://talon.ai/zen) にアクセスします。
...
2. **次のいずれかの方法を使用して認証を構成します**:
   ...
   ***
   ...
   プロジェクト固有の設定または永続的な設定の場合は、`talon.json` を使用します。
...
talon は、OpenAI 互換の API サーバーの背後でローカル LLM を実行するデスクトップアプリケーション [Atomic Chat](https://atomic.chat) 経由でローカルモデルを使うように設定できます（デフォルトのエンドポイントは `http://127.0.0.1:1337/v1`）。
...
`talon.json` ファイル。セッション共有を無効にすることもお勧めします。
...
[llama.cpp の s](https://github.com/ggml-org/llama.cpp) llama-server ユーティリティ] を通じて、ローカルモデルを使用するように talon を構成できます。
...
LM Studio を通じてローカルモデルを使用するように talon を構成できます。
...
Ollama を通じてローカルモデルを使用するように talon を構成できます。
...
4. プロジェクトディレクトリで `talon.json` ファイルを作成または更新します。
