...
ここでは、コミュニティが作成した OpenCode 用の CI/CD コンポーネント — [nagyv/gitlab-talon](https://gitlab.com/nagyv/gitlab-talon).
...
さらに多くの入力と使用例については、このコンポーネントの [ドキュメント](https://gitlab.com/explore/catalog/nagyv/gitlab-talon) をチェックしてください。
...
コメントで `@talon` に言及すると、OpenCode が GitLab CI パイプライン内でタスクを実行します。
...
:::tip
`@talon` とは異なるトリガーフレーズを使用するように設定できます。
:::
