...
デフォルトでは、OpenCode は独自の `talon` テーマを使用します。
...
2. **ユーザー設定ディレクトリ** - `~/.config/talon/themes/*.json` または `$XDG_CONFIG_HOME/talon/themes/*.json` で定義されます
3. **プロジェクトのルートディレクトリ** - `<project-root>/.talon/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.talon/themes/*.json` で定義
...
1. **組み込みテーマ** - これらはバイナリに埋め込まれています
2. **ユーザー設定ディレクトリ** - `~/.config/talon/themes/*.json` または `$XDG_CONFIG_HOME/talon/themes/*.json` で定義されます
3. **プロジェクトのルート ディレクトリ** - `<project-root>/.talon/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.talon/themes/*.json` で定義
