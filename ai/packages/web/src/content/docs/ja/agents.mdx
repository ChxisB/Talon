...
`talon.json` 設定ファイルでエージェントを構成します。
...
- グローバル: `~/.config/talon/agents/`
- プロジェクトごと: `.talon/agents/`
...
OpenCode 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[OpenCode Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `talon/gpt-5.1-codex` を使用します。
...
:::tip
`talon models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/ChxisB/Talon) を送信します。
:::
