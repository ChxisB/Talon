---
title: 伺服器
description: 透過 HTTP 與 talon 伺服器互動。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`talon serve` 指令執行一個無介面的 HTTP 伺服器，暴露一個 OpenAPI 端點供 talon 客戶端使用。
...
| `--mdns-domain` | mDNS 服務的自訂網域名稱 | `talon.local` |
| `--cors`        | 額外允許的瀏覽器來源    | `[]`             |
...
設定 `TALON_SERVER_PASSWORD` 以使用 HTTP 基本認證保護伺服器。使用者名稱預設為 `talon`，也可以設定 `TALON_SERVER_USERNAME` 來覆蓋它。這適用於 `talon serve` 和 `talon web`。
...
當你執行 `talon` 時，它會啟動一個 TUI 和一個伺服器。TUI 是與伺服器通訊的客戶端。伺服器暴露一個 OpenAPI 3.1 規範端點。該端點也用於產生 [SDK](/docs/sdk)。

:::tip
使用 talon 伺服器以程式化方式與 talon 互動。
:::

這種架構讓 talon 支援多個客戶端，並允許你以程式化方式與 talon 互動。

你可以執行 `talon serve` 來啟動一個獨立的伺服器。如果你已經在執行 talon TUI，`talon serve` 會啟動一個新的伺服器。
