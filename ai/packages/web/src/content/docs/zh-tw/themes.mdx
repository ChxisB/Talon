...
預設情況下，OpenCode 使用我們自己的 `talon` 主題。
...
1. **內建主題** — 嵌入在二進位檔案中
2. **使用者設定目錄** — 定義在 `~/.config/talon/themes/*.json` 或 `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **專案根目錄** — 定義在 `<project-root>/.talon/themes/*.json`
4. **當前工作目錄** — 定義在 `./.talon/themes/*.json`
