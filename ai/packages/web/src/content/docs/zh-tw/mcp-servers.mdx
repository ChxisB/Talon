...
您可以在 [OpenCode 設定](https://talon.ai/docs/config/)的 `mcp` 欄位下定義 MCP 伺服器。為每個 MCP 指定一個唯一的名稱，在提示詞中可以透過該名稱來參照對應的 MCP。
...
組織可以透過其 `.well-known/talon` 端點提供預設的 MCP 伺服器。這些伺服器可能預設處於停用狀態，允許使用者按需啟用。
...
如果伺服器需要身分驗證，OpenCode 會在您首次使用時提示您進行認證。您也可以使用 `talon mcp auth <server-name>` [手動觸發認證流程](#authenticating)。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，OpenCode 會將 Token 安全地儲存在 `~/.local/share/talon/mcp-auth.json` 中。
