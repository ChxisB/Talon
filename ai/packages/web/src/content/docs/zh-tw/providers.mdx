...
1. 在 TUI 中執行 `/connect` 指令，選擇 `OpenCode Zen`，然後前往 [talon.ai/auth](https://talon.ai/zen)。
...
1. 在 TUI 中執行 `/connect` 指令，選擇 `OpenCode Go`，然後前往 [talon.ai/auth](https://talon.ai/zen)。
...
你可以透過 [Atomic Chat](https://atomic.chat) 設定 talon 以使用本地模型。Atomic Chat 是一款桌面應用程式，它在 OpenAI 相容的 API 伺服器後方執行本地 LLM（預設端點 `http://127.0.0.1:1337/v1`）。
...
:::note[合規說明]
OpenCode 會使用一個小模型來執行部分 AI 任務，例如產生工作階段標題。預設情況下使用由 Zen 託管的 gpt-5-nano。如果您需要讓 OpenCode 僅使用您自己的 GitLab 託管實例，請在 `talon.json` 檔案中新增以下內容。同時建議停用工作階段分享。
