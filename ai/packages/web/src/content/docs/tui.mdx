...
This is separate from `talon.json`, which configures server/runtime behavior.
...
Use `TALON_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `talon.default`.
