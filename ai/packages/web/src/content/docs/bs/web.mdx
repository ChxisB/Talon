...
:::caution
Ako `TALON_SERVER_PASSWORD` nije postavljen, server će biti nezaštićen. Ovo je u redu za lokalnu upotrebu, ali bi trebalo biti postavljeno za pristup mreži.
:::

:::tip[Windows korisnici]
Za najbolje iskustvo, pokrenite `talon web` iz [WSL](/docs/windows-wsl) umjesto PowerShell-a. Ovo osigurava pravilan pristup sistemu datoteka i integraciju terminala.
:::
...
Ovo automatski postavlja ime hosta na `0.0.0.0` i oglašava server kao `talon.local`.
...
Da biste zaštitili pristup, postavite lozinku koristeći varijablu okruženja `TALON_SERVER_PASSWORD`:

```bash
TALON_SERVER_PASSWORD=secret talon web
```

Korisničko ime podrazumevano je `talon`, ali se može promeniti sa `TALON_SERVER_USERNAME`.
...
Također možete konfigurirati postavke servera u svom `talon.json` konfiguracijskom fajlu:
