...
- `.talon/plugins/` - Dodaci na nivou projekta
- `~/.config/talon/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/talon/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/talon/talon.json`)
2. Konfiguracija projekta (`talon.json`)
3. Globalni direktorij dodataka (`~/.config/talon/plugins/`)
4. Direktorij dodataka projekta (`.talon/plugins/`)
...
Spriječite talon da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u talon:
...
Pomoćnik `tool` kreira prilagođeni alat koji talon može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za talon zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://talon.ai/docs/sdk) za detalje.
