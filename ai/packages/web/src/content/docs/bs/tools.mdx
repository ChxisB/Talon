...
:::note
Ovaj alat je dostupan samo kada je `TALON_EXPERIMENTAL_LSP_TOOL=true` (ili `TALON_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCode provajdera ili kada je varijabla `TALON_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).
