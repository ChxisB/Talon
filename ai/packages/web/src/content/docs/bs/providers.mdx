...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite talon i idite na [talon.ai/auth](https://talon.ai/auth).
...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenCode Go` i idite na [talon.ai/auth](https://talon.ai/zen).
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `talon.json`:
...
Možete konfigurirati talon za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi talon ispravno radio.
   :::
...
Također možete dodati modele kroz svoju talon konfiguraciju.
...
da pohrani token u talon auth memoriju.
:::
...
`talon.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Možete konfigurirati talon za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati talon za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati talon za korištenje lokalnih modela putem Ollame.
...
Također možete dodati dodatne modele putem vaše talon konfiguracije.
...
5. Također ih možete prilagoditi putem vaše talon konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju talon konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `talon.json` datoteku u direktoriju projekta:
...
2. Za prilagođene provajdere, provjerite OpenCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj talon konfiguraciji.
