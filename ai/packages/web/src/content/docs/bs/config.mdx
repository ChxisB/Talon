...
1. **Udaljena konfiguracija** (od `.well-known/talon`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/talon/talon.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`TALON_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`talon.json` u projektu) - postavke specifične za projekat
5. **`.talon` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`TALON_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.talon` i `~/.config/talon` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/talon` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/talon/talon.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/talon/tui.json`.
...
Dodajte `talon.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `TALON_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `TALON_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.talon` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.talon`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`talon.ai/config.json`**](https://talon.ai/config.json).

TUI konfiguracija koristi [**`talon.ai/tui.json`**](https://talon.ai/tui.json).
...
Koristite `TALON_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `talon.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `talon serve` i `talon web` putem opcije `server`.
...
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `talon.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/talon/agents/` ili `.talon/agents/`. [Saznajte više ovdje](/docs/agents).
...
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`talon run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/talon/commands/` ili `.talon/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.talon/plugins/` ili `~/.config/talon/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
