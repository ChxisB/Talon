...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\talon\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `talon --log-level DEBUG`.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop pokreće lokalni OpenCode server (`talon-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (ili `~/.config/talon/talon.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/talon/talon.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Globalni dodaci**
  ...
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\talon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/talon`
- **Linux**: obrišite `~/.cache/talon` (ili pokrenite `rm -rf ~/.cache/talon`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\talon`
...
Ako vaš `talon.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `TALON_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `TALON_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
- `talon.settings.dat` (URL zadanog servera za desktop)
- `talon.global.dat` i `talon.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `talon upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `talon models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\talon`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\talon`
