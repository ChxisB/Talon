---
title: Server
description: Komunicirajte s talon serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `talon serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji talon klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `talon.local` |
...
Postavite `TALON_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `talon`, ili postavite `TALON_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `talon serve` i za `talon web`.
...
Kada pokrenete `talon`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite talon server za programsku interakciju sa talon-om.
:::

Ova arhitektura omogucava talon podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `talon serve` da startate standalone server. Ako je talon TUI vec pokrenut, `talon serve` ce pokrenuti novi server.
...
talon server izlaže sljedece API-je.
