...
Po defaultu, OpenCode koristi nasu `talon` temu.
...
2. **Korisnicki config direktorij** - `~/.config/talon/themes/*.json` ili `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.talon/themes/*.json`
4. **Trenutni radni direktorij** - `./.talon/themes/*.json`
