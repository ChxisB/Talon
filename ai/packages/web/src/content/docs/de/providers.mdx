...
1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenCode aus und gehen Sie zu [talon.ai/auth](https://talon.ai/auth).
...
1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenCode Go` und gehen Sie zu [talon.ai/auth](https://talon.ai/zen).
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `talon.json`:
...
Sie können talon so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
`talon.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
4. Erstellen oder aktualisieren Sie Ihre `talon.json`-Datei in Ihrem Projektverzeichnis:
