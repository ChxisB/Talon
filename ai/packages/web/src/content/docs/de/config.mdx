...
Wenn Ihre globale Konfiguration beispielsweise `theme: "talon"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/talon`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/talon/talon.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`TALON_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`talon.json` im Projekt) – projektspezifische Einstellungen
5. **`.talon` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`TALON_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
:::note
Die Verzeichnisse `.talon` und `~/.config/talon` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/talon` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/talon/talon.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `talon.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Geben Sie mithilfe der Umgebungsvariablen `TALON_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `TALON_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
Umgebungsvariable. Dieses Verzeichnis wird nach Agenten, Befehlen,
Modi und Plugins genau wie das Standardverzeichnis `.talon` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.talon` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`talon.ai/config.json`**](https://talon.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `talon serve` und `talon web` über die Option `server` konfigurieren.
...
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `talon.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/talon/agents/` oder `.talon/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`talon run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/talon/commands/` oder `.talon/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.talon/plugins/` oder `~/.config/talon/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
