...
OpenCode lässt sich in VS Code, Cursor oder jede IDE integrieren, die ein Terminal unterstützt. Führen Sie einfach `talon` im Terminal aus, um zu beginnen.
...
3. Führen Sie `talon` aus – die Erweiterung wird automatisch installiert
...
- Stellen Sie sicher, dass Sie `talon` im integrierten Terminal ausführen.
