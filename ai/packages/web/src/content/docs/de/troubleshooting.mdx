...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\talon\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `talon --log-level DEBUG`.

---

## Speicher

talon speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop startet im Hintergrund einen lokalen OpenCode-Server (`talon-cli`-Sidecar).
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (oder `~/.config/talon/talon.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/talon/talon.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Globale Plugins**
  ...
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\talon\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/talon`
- **Linux**: loesche `~/.cache/talon` (oder fuehre aus: `rm -rf ~/.cache/talon`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\talon`
...
Wenn deine `talon.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `TALON_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `TALON_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
- `talon.global.dat` und `talon.workspace.*.dat` (UI state like recent servers/projects)
...
3. Stelle sicher, dass du die neueste Version hast: `talon upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `talon models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\talon`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. talon installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\talon`

2. Starte talon neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt talon, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
talon erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
