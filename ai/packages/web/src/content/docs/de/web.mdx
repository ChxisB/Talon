...
:::caution
Wenn `TALON_SERVER_PASSWORD` nicht gesetzt ist, ist der Server ungesichert. Dies ist für den lokalen Gebrauch in Ordnung, sollte aber für den Netzwerkzugriff eingestellt werden.
:::

:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `talon web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Dadurch wird der Hostname automatisch auf `0.0.0.0` gesetzt und der Server als `talon.local` angekündigt.
...
Um den Zugriff zu schützen, legen Sie mithilfe der Umgebungsvariablen `TALON_SERVER_PASSWORD` ein Passwort fest:

```bash
TALON_SERVER_PASSWORD=secret talon web
```

Der Benutzername ist standardmäßig `talon`, kann aber mit `TALON_SERVER_USERNAME` geändert werden.
...
Sie können Servereinstellungen auch in Ihrer `talon.json`-Konfigurationsdatei konfigurieren:
