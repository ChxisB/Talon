...
OpenCode lässt sich in Ihren GitHub-Workflow integrieren. Erwähnen Sie `/talon` oder `/oc` in Ihrem Kommentar, und OpenCode führt Aufgaben in Ihrem GitHub Actions-Runner aus.
...
1. **Installieren Sie die GitHub-App**
   ...
   Gehen Sie zu [**github.com/apps/talon-agent**](https://github.com/apps/talon-agent). Stellen Sie sicher, dass es im Ziel-Repository installiert ist.

2. **Den Workflow hinzufügen**

   Fügen Sie die folgende Workflowdatei zu `.github/workflows/talon.yml` in Ihrem Repository hinzu. Stellen Sie sicher, dass Sie in `env` die passenden Werte für `model` und `api_key` setzen.
...
| ----------------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issue_comment`               | Kommentieren Sie ein Problem oder PR              | Erwähnen Sie `/talon` oder `/oc` in Ihrem Kommentar. OpenCode liest den Kontext und kann Branches erstellen, PRs öffnen oder antworten.          |
| `pull_request_review_comment` | Kommentieren Sie bestimmte Codezeilen in einem PR | Erwähnen Sie `/talon` oder `/oc` beim Überprüfen des Codes. OpenCode empfängt Dateipfad, Zeilennummern und Diff Context.                         |
