---
title: Regeln
description: Lege eigene Anweisungen fuer talon fest.
---

Du kannst talon ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in talon den Befehl `/init` aus.
...
So kann talon sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht talon sie zu erweitern.
...
talon kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/talon/AGENTS.md` speichern.
Diese gelten in allen talon-Sitzungen.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/talon/AGENTS.md` existiert)
...
Beim Start sucht talon in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/talon/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `talon.json` oder in `~/.config/talon/talon.json` angeben.
...
talon parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit talon.json

Empfohlen ist das `instructions`-Feld in der `talon.json`:
...
Du kannst talon explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `talon.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
