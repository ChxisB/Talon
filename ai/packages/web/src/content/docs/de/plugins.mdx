...
- `.talon/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/talon/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/talon/talon.json`)
2. Projektkonfiguration (`talon.json`)
3. Globales Plugin-Verzeichnis (`~/.config/talon/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.talon/plugins/`)
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://talon.ai/docs/sdk).
