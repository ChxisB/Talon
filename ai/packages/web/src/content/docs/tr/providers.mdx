...
talon, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı talon yapılandırmanızda yapılandırın.
...
`~/.local/share/talon/auth.json` içinde.
...
OpenCode Zen, talon ekibi tarafından sağlanan modellerin bir listesidir.
talon ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, opencode'u seçin ve [talon.ai/auth](https://talon.ai/auth)'ye gidin.
...
OpenCode Go, talon ile iyi çalıştığı test edilmiş ve doğrulanmış, talon ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenCode Go`'yu seçin ve [talon.ai/auth](https://talon.ai/zen) adresine gidin.
...
Amazon Bedrock'u talon ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `talon.json` kullanın:
...
```json title="talon.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://talon.ai/config.json",
  "provider": {
    "atomic-chat": {
...
to store token in talon auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
talon, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
...
`talon.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`talon.json` üzerinden özelleştirin:
...
Google Vertex AI'yi talon ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları talon çalıştırılırken ayarlayın.
...
Helicone'un talon aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
```jsonc title="~/.config/talon/talon.jsonc"
{
  "$schema": "https://talon.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her talon görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her talon görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
```json title="talon.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://talon.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```json title="talon.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://talon.ai/config.json",
  "provider": {
    "lmstudio": {
...
:::tip
Ollama kendisini talon için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::

```json title="talon.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://talon.ai/config.json",
  "provider": {
    "ollama": {
...
Ollama Cloud'u talon ile kullanmak için:
...
OpenCode Zen, talon ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. talon API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   talon yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca talon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri talon yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı talon ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
3. Sağlayıcı için API anahtarınızı girin.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in talon.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Proje dizininizde `talon.json` dosyanızı oluşturun veya güncelleyin:

   ```json title="talon.json" ""myprovider"" {5-15}
   {
     "$schema": "https://talon.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="talon.json" {9,11,17-20}
{
  "$schema": "https://talon.ai/config.json",
  "provider": {
    "myprovider": {
...
2. Özel sağlayıcılar için talon yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, talon yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
