...
- `.talon/plugins/` - Proje düzeyinde eklentiler
- `~/.config/talon/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/talon/node_modules/`'da önbelleğe alınır.
...
3. Global eklenti dizini (`~/.config/talon/plugins/`)
4. Proje eklenti dizini (`.talon/plugins/`)
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://talon.ai/docs/sdk) bakın.
