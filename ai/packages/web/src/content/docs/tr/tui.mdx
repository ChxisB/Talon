...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `talon.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `TALON_TUI_CONFIG` kullanın.
...
- `sound_pack` - Kullanılacak sound pack ID. Varsayılan değer `talon.default`.
