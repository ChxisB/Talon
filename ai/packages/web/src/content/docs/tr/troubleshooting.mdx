...
talon ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\talon\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `talon --log-level DEBUG`.

---

## Depolama

talon, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\talon` yapistirin
...
talon Desktop arka planda yerel bir talon sunucusu (`talon-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `talon` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (veya `~/.config/talon/talon.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/talon/talon.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\talon\talon.jsonc` yapistirin
...
talon diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/talon/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\talon\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), talon'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. talon Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/talon`
- **Linux**: `~/.cache/talon` dizinini silin (veya `rm -rf ~/.cache/talon` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\talon` yapistirin

3. talon Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

talon Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`talon.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `TALON_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `TALON_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta talon Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, talon ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

talon Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda talon icin bildirimler etkinse
...
1. talon Desktop'u kapatin
2. Su dosyalari bulun ve silin (talon Desktop uygulama veri dizininde yer alirlar):

- `talon.settings.dat` (desktop varsayilan sunucu URL'si)
- `talon.global.dat` ve `talon.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
talon ile ilgili bir sorun yasiyorsaniz:
...
3. `talon upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `talon models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\talon`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. talon, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\talon`

2. En guncel provider paketlerini yeniden kurmak icin talon'u yeniden baslatin

Bu, talon'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
talon, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
