...
_Model Bağlam Protokolü_ veya MCP kullanarak talon'a harici araçlar ekleyebilirsiniz. talon hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [talon Config](https://talon.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.

```jsonc title="talon.jsonc" {6}
{
...
Kuruluşlar, `.well-known/talon` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
talon, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde talon şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa talon, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `talon mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
```bash
talon mcp auth my-oauth-server
```
...
```bash
talon mcp logout my-oauth-server
```

`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından talon, belirteçleri `~/.local/share/talon/mcp-auth.json`'de güvenli bir şekilde saklar.
...
talon mcp debug my-oauth-server
```
...
MCP'leriniz yerleşik araçların yanı sıra talon'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi talon yapılandırması aracılığıyla yönetebilirsiniz.
...
```json title="talon.json" {11, 14-18}
{
  "$schema": "https://talon.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
talon mcp auth sentry
```

Bu, OAuth akışını tamamlamak ve talon'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
