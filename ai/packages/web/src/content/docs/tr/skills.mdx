...
Ajan becerileri, talon'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
talon su konumlari tarar:

- Proje konfigurasyonu: `.talon/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/talon/skills/<name>/SKILL.md`
...
Proje yerel yollarinda talon, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.talon/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/talon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.talon/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
talon, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`talon.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`talon.json` icinde):
