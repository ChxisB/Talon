...
- `.talon/plugins/` - Plugins au niveau du projet
- `~/.config/talon/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/talon/node_modules/`.
...
1. Configuration globale (`~/.config/talon/talon.json`)
2. Configuration du projet (`talon.json`)
3. Répertoire global des plugins (`~/.config/talon/plugins/`)
4. Répertoire des plugins du projet (`.talon/plugins/`)
...
- `client` : un client SDK talon pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre talon.
...
Empêchez talon de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à talon :
...
L'assistant `tool` crée un outil personnalisé que talon peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour talon aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://talon.ai/docs/sdk) pour plus de détails.
