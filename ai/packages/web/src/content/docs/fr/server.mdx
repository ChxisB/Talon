---
title: Serveur
description: Interagissez avec le serveur talon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `talon serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client talon peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `talon.local` |
...
Définissez `TALON_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `talon`, ou définissez `TALON_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `talon serve` et à `talon web`.
...
Lorsque vous exécutez `talon`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur talon pour interagir avec talon par programmation.
:::

Cette architecture permet à talon de prendre en charge plusieurs clients et vous permet d'interagir avec talon par programme.

Vous pouvez exécuter `talon serve` pour démarrer un serveur autonome. Si vous avez le
talon TUI en cours d'exécution, `talon serve` démarrera un nouveau serveur.
...
Le serveur talon expose les API suivantes.
