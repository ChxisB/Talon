...
Par défaut, OpenCode utilise notre propre thème `talon`.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/talon/themes/*.json` ou `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.talon/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.talon/themes/*.json`
