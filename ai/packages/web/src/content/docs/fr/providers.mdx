...
en `~/.local/share/talon/auth.json`.
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez talon et dirigez-vous vers [talon.ai/auth](https://talon.ai/auth).
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenCode Go` et rendez-vous sur [talon.ai/auth](https://talon.ai/zen).
...
Pour une configuration spécifique au projet ou persistante, utilisez `talon.json` :
...
Vous pouvez configurer talon pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que talon fonctionne correctement.
   :::
...
Fichier `talon.json`. Il est également recommandé de désactiver le partage de session.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via talon, vous pouvez toujours le configurer vous-même.
...
Vous pouvez configurer talon pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer talon pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer talon pour utiliser des modèles locaux via Ollama.
...
4. Créez ou mettez à jour votre fichier `talon.json` dans le répertoire de votre projet :
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration talon et :
