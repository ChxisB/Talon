...
:::note
Cet outil n'est disponible que lorsque `TALON_EXPERIMENTAL_LSP_TOOL=true` (ou `TALON_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur OpenCode ou lorsque la variable d'environnement `TALON_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).
