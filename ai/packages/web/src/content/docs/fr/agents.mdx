...
Configurez les agents dans votre fichier de configuration `talon.json` :
...
L'ID de modèle dans votre configuration OpenCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenCode Zen](/docs/zen), vous utiliserez `talon/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `talon models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/ChxisB/Talon).
:::
