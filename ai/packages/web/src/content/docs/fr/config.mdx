...
1. **Configuration distante** (à partir de `.well-known/talon`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/talon/talon.json`) - préférences utilisateur
3. **Configuration personnalisée** (`TALON_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`talon.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.talon`** - agents, commandes, plugins
6. **Configuration en ligne** (`TALON_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.talon` et `~/.config/talon` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/talon`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale OpenCode dans `~/.config/talon/talon.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/talon/tui.json`.
...
Ajoutez `talon.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `TALON_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `TALON_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.talon`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.talon`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`talon.ai/config.json`**](https://talon.ai/config.json).

La configuration TUI utilise [**`talon.ai/tui.json`**](https://talon.ai/tui.json).
...
Utilisez `TALON_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `talon.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `talon serve` et `talon web` via l'option `server`.
...
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `talon.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/talon/agents/` ou `.talon/agents/`. [En savoir plus ici](/docs/agents).
...
Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`talon run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/talon/commands/` ou `.talon/commands/`. [En savoir plus ici](/docs/commands).
...
Par défaut, talon **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
Placez les fichiers du plugin dans `.talon/plugins/` ou `~/.config/talon/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
