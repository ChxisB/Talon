...
OpenCode s'intègre à VS Code, Cursor ou tout IDE prenant en charge un terminal. Exécutez simplement `talon` dans le terminal pour commencer.
...
3. Exécutez `talon` - l'extension s'installe automatiquement
...
- Assurez-vous que vous exécutez `talon` dans le terminal intégré.
