...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\talon\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `talon --log-level DEBUG`.

---

## Stockage

talon stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/talon/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop exécute un serveur OpenCode local (le side-car `talon-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/talon/talon.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Plugins mondiaux**
  ...
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\talon\plugins`
...
- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/talon`
- **Linux** : supprimez `~/.cache/talon` (ou exécutez `rm -rf ~/.cache/talon`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\talon`
...
Si votre `talon.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `TALON_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `TALON_PORT` (ou choisissez un port libre) et redémarrez.
...
- `talon.settings.dat` (serveur de bureau par défaut URL)
- `talon.global.dat` et `talon.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
3. Assurez-vous d'avoir la dernière version avec `talon upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `talon models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\talon`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. talon installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\talon`

2. Redémarrez talon pour réinstaller les derniers packages du fournisseur

Cela forcera talon à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
talon détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
