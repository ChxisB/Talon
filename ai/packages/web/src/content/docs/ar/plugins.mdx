...
- `.talon/plugins/` - إضافات على مستوى المشروع
- `~/.config/talon/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/talon/node_modules/`.
...
1. إعدادات عامة (`~/.config/talon/talon.json`)
2. إعدادات المشروع (`talon.json`)
3. دليل الإضافات العام (`~/.config/talon/plugins/`)
4. دليل إضافات المشروع (`.talon/plugins/`)
...
- `client`: عميل SDK لـ talon للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع talon.
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى talon:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ talon استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ talon إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://talon.ai/docs/sdk) للتفاصيل.
