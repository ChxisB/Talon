...
في `~/.local/share/talon/auth.json`.
...
1. شغّل الأمر `/connect` في TUI، واختر opencode، ثم انتقل إلى [talon.ai/auth](https://talon.ai/auth).
...
1. شغّل الأمر `/connect` في TUI، واختر `OpenCode Go`، ثم انتقل إلى [talon.ai/auth](https://talon.ai/zen).
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `talon.json`:
...
يمكنك تكوين talon لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل talon بشكل صحيح.
   :::
...
`talon.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
يمكنك ضبط talon لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط talon لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط talon لاستخدام نماذج محلية عبر Ollama.
...
4. أنشئ أو حدّث ملف `talon.json` في دليل مشروعك:
...
2. للمزوّدات المخصّصة، راجع إعدادات talon وتأكد من:
