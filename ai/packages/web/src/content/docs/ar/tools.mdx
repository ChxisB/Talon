...
:::note
هذه الأداة متاحة فقط عند ضبط `TALON_EXPERIMENTAL_LSP_TOOL=true` (أو `TALON_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود OpenCode أو عند ضبط متغير البيئة `TALON_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).
