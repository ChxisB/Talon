...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "talon"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/talon`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/talon/talon.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`TALON_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`talon.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.talon`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`TALON_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.talon` و`~/.config/talon` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/talon`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات OpenCode العامة في `~/.config/talon/talon.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `talon.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `TALON_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `TALON_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.talon`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.talon`، لذلك **يمكنه تجاوز** إعداداتها.

---

## المخطط

يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`talon.ai/config.json`**](https://talon.ai/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `talon serve` و`talon web` عبر الخيار `server`.
...
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `talon.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/talon/agents/` أو `.talon/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`talon run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/talon/commands/` أو `.talon/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
افتراضيًا يسمح talon **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
ضع ملفات الإضافات في `.talon/plugins/` أو `~/.config/talon/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
