...
افتراضيًا، يستخدم OpenCode سمتنا `talon`.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/talon/themes/*.json` أو `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.talon/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.talon/themes/*.json`
