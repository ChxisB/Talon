---
title: الخادم
description: تفاعل مع خادم talon عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `talon serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل talon استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `talon.local` |
...
عيّن `TALON_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `talon`، أو عيّن `TALON_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `talon serve` و `talon web`.
...
عند تشغيل `talon` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم talon للتفاعل مع talon برمجيا.
:::

تتيح هذه البنية لـ talon دعم عدة عملاء وتمكّنك من التفاعل مع talon برمجيا.

يمكنك تشغيل `talon serve` لبدء خادم مستقل. إذا كانت واجهة talon في terminal (TUI)
قيد التشغيل، فسيبدأ `talon serve` خادما جديدا.
...
يعرض خادم talon واجهات API التالية.
