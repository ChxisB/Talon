...
![使用 talon 主题的 OpenCode TUI](../../../assets/lander/screenshot.png)
...
- **在 macOS 和 Linux 上使用 Homebrew**
  ...
  > 我们推荐使用 OpenCode tap 以获取最新版本。官方的 `brew install talon` formula 由 Homebrew 团队维护，更新频率较低。
...
你也可以从 [Releases](https://github.com/ChxisB/Talon/releases) 页面直接下载二进制文件。
...
1. 在 TUI 中运行 `/connect` 命令，选择 talon，然后前往 [talon.ai/auth](https://talon.ai/auth)。
...
这是一个与 OpenCode 的[示例对话](https://talon.ai/s/4XP1fce5)。
