...
- 项目配置：`.talon/skills/<name>/SKILL.md`
- 全局配置：`~/.config/talon/skills/<name>/SKILL.md`
...
在此过程中，它会加载 `.talon/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。

全局定义也会从 `~/.config/talon/skills/*/SKILL.md`、`~/.claude/skills/*/SKILL.md` 和 `~/.agents/skills/*/SKILL.md` 中加载。
...
创建 `.talon/skills/git-release/SKILL.md`，内容如下：
...
在 `talon.json` 中使用基于模式的权限来控制代理可以访问哪些技能：
