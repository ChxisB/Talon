...
:::caution
如果未设置 `TALON_SERVER_PASSWORD`，服务器将没有安全保护。本地使用没有问题，但在网络访问时应当设置密码。
:::

:::tip[Windows 用户]
为获得最佳体验，建议从 [WSL](/docs/windows-wsl) 而非 PowerShell 运行 `talon web`。这可以确保正确的文件系统访问和终端集成。
:::
...
这会自动将主机名设置为 `0.0.0.0`，并将服务器广播为 `talon.local`。
...
要保护服务器访问，可以通过 `TALON_SERVER_PASSWORD` 环境变量设置密码：

```bash
TALON_SERVER_PASSWORD=secret talon web
```

用户名默认为 `talon`，可以通过 `TALON_SERVER_USERNAME` 进行更改。
