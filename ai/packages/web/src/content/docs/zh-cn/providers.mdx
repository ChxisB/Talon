...
1. 在 TUI 中执行 `/connect` 命令，选择 opencode，然后前往 [talon.ai/auth](https://talon.ai/auth)。
...
你可以通过 [Atomic Chat](https://atomic.chat) 配置 talon 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `http://127.0.0.1:1337/v1`）。
...
:::note[合规说明]
OpenCode 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 OpenCode 仅使用你自己的 GitLab 托管实例，请在 `talon.json` 文件中添加以下内容。同时建议禁用会话共享。
