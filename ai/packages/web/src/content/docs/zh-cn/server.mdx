---
title: 服务器
description: 通过 HTTP 与 talon 服务器交互。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`talon serve` 命令运行一个无界面的 HTTP 服务器，暴露一个 OpenAPI 端点供 talon 客户端使用。
...
| `--mdns-domain` | mDNS 服务的自定义域名 | `talon.local` |
| `--cors`        | 额外允许的浏览器来源  | `[]`             |
...
设置 `TALON_SERVER_PASSWORD` 以使用 HTTP 基本认证保护服务器。用户名默认为 `talon`，也可以设置 `TALON_SERVER_USERNAME` 来覆盖它。这适用于 `talon serve` 和 `talon web`。
...
当你运行 `talon` 时，它会启动一个 TUI 和一个服务器。TUI 是与服务器通信的客户端。服务器暴露一个 OpenAPI 3.1 规范端点。该端点也用于生成 [SDK](/docs/sdk)。

:::tip
使用 talon 服务器以编程方式与 talon 交互。
:::

这种架构让 talon 支持多个客户端，并允许你以编程方式与 talon 交互。

你可以运行 `talon serve` 来启动一个独立的服务器。如果你已经在运行 talon TUI，`talon serve` 会启动一个新的服务器。
