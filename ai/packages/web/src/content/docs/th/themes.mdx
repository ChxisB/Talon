...
ตามค่าเริ่มต้น OpenCode จะใช้ธีม `talon` ของเราเอง
...
2. **ไดเรกทอรีกำหนดค่าผู้ใช้** - กำหนดใน `~/.config/talon/themes/*.json` หรือ `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **ไดเรกทอรีรากของโครงการ** - กำหนดไว้ใน `<project-root>/.talon/themes/*.json`
4. **ไดเร็กทอรีการทำงานปัจจุบัน** - กำหนดใน `./.talon/themes/*.json`
