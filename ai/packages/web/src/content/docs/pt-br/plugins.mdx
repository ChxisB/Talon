---
title: Plugins
description: Escreva seus próprios plugins para estender o talon.
---

Plugins permitem que você estenda o talon conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do talon.
...
- `.talon/plugins/` - Plugins em nível de projeto
- `~/.config/talon/plugins/` - Plugins globais
...
```json title="talon.json"
{
  ...
  "plugin": ["talon-helicone-session", "talon-wakatime", "@my-org/custom-plugin"]
}
```
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/talon/node_modules/`.
...
1. Configuração global (`~/.config/talon/talon.json`)
2. Configuração do projeto (`talon.json`)
3. Diretório de plugins global (`~/.config/talon/plugins/`)
4. Diretório de plugins do projeto (`.talon/plugins/`)
...
O talon executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.

```ts title=".talon/plugins/my-plugin.ts"
...
```js title=".talon/plugins/example.js"
...
- `client`: Um cliente SDK do talon para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o talon.
...
```js title=".talon/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "talon"'`
      }
    },
  }
}
...
:::note
Se você estiver usando o aplicativo desktop talon, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o talon de ler arquivos `.env`:

```javascript title=".talon/plugins/env-protection.js"
...
```javascript title=".talon/plugins/inject-env.js"
...
Plugins também podem adicionar ferramentas personalizadas ao talon:

```ts title=".talon/plugins/custom-tools.ts"
import { type Plugin, tool } from "@talon-ai/plugin"
...
O helper `tool` cria uma ferramenta personalizada que o talon pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o talon junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://talon.ai/docs/sdk) para detalhes.
...
```ts title=".talon/plugins/compaction.ts"
import type { Plugin } from "@talon-ai/plugin"
...
```ts title=".talon/plugins/custom-compaction.ts"
import type { Plugin } from "@talon-ai/plugin"
