...
Com o talon, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o talon usa nosso próprio tema `talon`.
...
O talon vem com vários temas embutidos.
...
- Querem que o talon corresponda à aparência do seu terminal
...
O talon suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/talon/themes/*.json` ou `$XDG_CONFIG_HOME/talon/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.talon/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.talon/themes/*.json`
