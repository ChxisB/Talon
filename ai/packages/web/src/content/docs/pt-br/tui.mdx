---
title: TUI
description: Usando a interface de usuário de terminal talon.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O talon fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar talon inicia o TUI para o diretório atual.
...
Ao usar o TUI do talon, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao talon. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do talon. _Aliases_: `/quit`, `/q`
...
Isso é separado do `talon.json`, que configura o comportamento do servidor/runtime.
...
Use `TALON_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `talon.default`.
