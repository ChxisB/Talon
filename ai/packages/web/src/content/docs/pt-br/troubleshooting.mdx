...
Para depurar problemas com o talon, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **macOS/Linux**: `~/.local/share/talon/log/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\talon\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `talon --log-level DEBUG`.

---

## Armazenamento

O talon armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\talon`
...
O talon Desktop executa um servidor talon local (o sidecar `talon-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (ou `~/.config/talon/talon.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/talon/talon.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\talon\talon.jsonc`
...
O talon também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/talon/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\talon\plugins`
- **Plugins de projeto** (apenas se você usar configuração por projeto)
  - `<your-project>/.talon/plugins/`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o talon possa reconstruí-lo.

1. Saia completamente do talon Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/talon`
- **Linux**: exclua `~/.cache/talon` (ou execute `rm -rf ~/.cache/talon`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\talon`

3. Reinicie o talon Desktop.

---

### Corrigindo problemas de conexão com o servidor

O talon Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `talon.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `TALON_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `TALON_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o talon Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do talon.

---

### Notificações não aparecendo

O talon Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o talon nas configurações do seu sistema operacional, e
...
1. Saia do talon Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo talon Desktop):

- `talon.settings.dat` (URL do servidor padrão do desktop)
- `talon.global.dat` e `talon.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o talon:
...
1. **Relatar problemas no GitHub**
   ...
   [**github.com/ChxisB/Talon/issues**](https://github.com/ChxisB/Talon/issues)
...
2. **Junte-se ao nosso Discord**
   ...
   [**talon.ai/discord**](https://talon.ai/discord)
...
3. Certifique-se de que você tem a versão mais recente com `talon upgrade`
...
Para descobrir quais modelos você tem acesso, execute `talon models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   ```bash
   rm -rf ~/.local/share/talon
   ```

   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\talon`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O talon instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\talon`

2. Reinicie o talon para reinstalar os pacotes de provedores mais recentes

Isso forçará o talon a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O talon detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
