---
title: Servidor
description: Interaja com o servidor talon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `talon serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente talon pode usar.
...
```bash
talon serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `talon.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
talon serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `TALON_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `talon`, ou defina `TALON_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `talon serve` quanto ao `talon web`.

```bash
TALON_SERVER_PASSWORD=your-password talon serve
```
...
Quando você executa `talon`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor talon para interagir com o talon programaticamente.
:::

Essa arquitetura permite que o talon suporte múltiplos clientes e permite que você interaja com o talon programaticamente.

Você pode executar `talon serve` para iniciar um servidor autônomo. Se você tiver o TUI do talon em execução, `talon serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do talon [IDE](/docs/ide).
...
O servidor talon expõe as seguintes APIs.
