...
Existem dois tipos de agentes no talon; agentes primários e subagentes.
...
talon vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Configure os agentes em seu arquivo de configuração `talon.json`:
...
- Global: `~/.config/talon/agents/`
- Por projeto: `.talon/agents/`

```markdown title="~/.config/talon/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o talon usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do talon quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do talon usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `talon/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="talon.json" {3-6,9-12}
{
  "$schema": "https://talon.ai/config.json",
  "tools": {
    "write": true,
...
```json title="talon.json" {3-5,8-10}
{
  "$schema": "https://talon.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `talon models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/ChxisB/Talon).
:::
...
```markdown title="~/.config/talon/agents/docs-writer.md"
---
...
```markdown title="~/.config/talon/agents/security-auditor.md"
---
