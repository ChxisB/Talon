...
talon usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/talon/auth.json`.
...
OpenCode Zen é uma lista de modelos fornecidos pela equipe do talon que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OpenCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `OpenCode Zen` e acesse [talon.ai/zen](https://talon.ai/zen).
...
Funciona como qualquer outro provedor no talon e é completamente opcional.

---

## OpenCode Go

OpenCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do talon que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `OpenCode Go` e acesse [talon.ai/zen](https://talon.ai/zen).
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `talon.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no talon não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o talon para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o talon funcione corretamente.
   :::
...
talon usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
...
arquivo `talon.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `talon.json`:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do talon, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do talon como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do talon listada como uma sessão separada.
...
Você pode configurar o talon para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o talon para usar modelos locais através do LM Studio.
...
Você pode configurar o talon para usar modelos locais através do Ollama.
...
6. **Importante**: Antes de usar modelos em nuvem no talon, você deve puxar as informações do modelo localmente:
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no talon.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `talon.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o talon entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
2. Para provedores personalizados, verifique a configuração do talon e:
