...
As habilidades do agente permitem que o talon descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O talon pesquisa nesses locais:

- Configuração do projeto: `.talon/skills/<name>/SKILL.md`
- Configuração global: `~/.config/talon/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o talon sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.talon/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/talon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.talon/skills/git-release/SKILL.md` assim:
...
O talon lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `talon.json`:
...
**Para agentes embutidos** (em `talon.json`):
