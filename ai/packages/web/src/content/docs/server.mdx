---
title: Server
description: Interact with talon server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `talon serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an talon client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `talon.local` |
...
Set `TALON_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `talon`, or set `TALON_SERVER_USERNAME` to override it. This applies to both `talon serve` and `talon web`.
...
When you run `talon` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the talon server to interact with talon programmatically.
:::

This architecture lets talon support multiple clients and allows you to interact with talon programmatically.

You can run `talon serve` to start a standalone server. If you have the
talon TUI running, `talon serve` will start a new server.
...
The talon server exposes the following APIs.
