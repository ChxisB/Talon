...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\talon\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `talon --log-level DEBUG`.

---

## Almacenamiento

talon almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/talon/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\talon`
...
OpenCode Desktop ejecuta un servidor OpenCode local (el sidecar `talon-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- **macOS/Linux**: `~/.config/talon/talon.jsonc` (o `~/.config/talon/talon.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/talon/talon.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\talon\talon.jsonc`
...
- **Complementos globales**
  ...
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\talon\plugins`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/talon`
- **Linux**: eliminar `~/.cache/talon` (o ejecutar `rm -rf ~/.cache/talon`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\talon`
...
Si su `talon.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `TALON_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `TALON_PORT` (o elija un puerto libre) y reinicie.
...
- `talon.settings.dat` (URL del servidor predeterminado de escritorio)
- `talon.global.dat` y `talon.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `talon upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `talon models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\talon`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. talon instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\talon`

2. Reinicie talon para reinstalar los paquetes del proveedor más recientes.

Esto obligará a talon a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
talon detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
