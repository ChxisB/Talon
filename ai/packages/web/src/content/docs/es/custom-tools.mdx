...
description: Cree herramientas que LLM pueda llamar en talon.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de talon como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.talon/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/talon/tools/`.
...
```ts title=".talon/tools/database.ts" {1}
...
```ts title=".talon/tools/project.ts" {8}
import { tool } from "@talon-ai/plugin"
...
```python title=".talon/tools/add.py"
import sys
...
```ts title=".talon/tools/python-add.ts" {10}
import { tool } from "@talon-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".talon/tools/add.py")
