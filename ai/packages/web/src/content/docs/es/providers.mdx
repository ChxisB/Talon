...
1. Ejecute el comando `/connect` en TUI, seleccione talon y diríjase a [talon.ai/auth](https://talon.ai/auth).
...
1. Ejecute el comando `/connect` en TUI, seleccione `OpenCode Go` y diríjase a [talon.ai/auth](https://talon.ai/zen).
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `talon.json`:
...
Puedes configurar talon para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que talon funcione correctamente.
   :::
...
Archivo `talon.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `talon.json`:
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de talon, siempre podrá configurarlo usted mismo.
...
Puede configurar talon para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar talon para usar modelos locales a través de LM Studio.
...
Puedes configurar talon para usar modelos locales a través de Ollama.
...
4. Cree o actualice su archivo `talon.json` en el directorio de su proyecto:
...
2. Para proveedores personalizados, verifique la configuración talon y:
