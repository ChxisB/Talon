...
Por ejemplo, si su configuración global establece `theme: "talon"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/talon`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/talon/talon.json`) - preferencias del usuario
3. **Configuración personalizada** (`TALON_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`talon.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.talon`** - agentes, comandos, complementos
6. **Configuración en línea** (`TALON_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.talon` y `~/.config/talon` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/talon`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global OpenCode en `~/.config/talon/talon.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `talon.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `TALON_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `TALON_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.talon`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.talon`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`talon.ai/config.json`**](https://talon.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `talon serve` y `talon web` a través de la opción `server`.
...
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `talon.local`. Útil para ejecutar múltiples instancias en la misma red.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/talon/agents/` o `.talon/agents/`. [Más información aquí](/docs/agents).
...
Esta configuración se aplica en todas las interfaces: TUI, CLI (`talon run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/talon/commands/` o `.talon/commands/`. [Más información aquí](/docs/commands).
...
De forma predeterminada, talon **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Coloque los archivos de complemento en `.talon/plugins/` o `~/.config/talon/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
