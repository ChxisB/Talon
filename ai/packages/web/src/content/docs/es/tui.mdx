...
Esto es independiente de `talon.json`, que configura el comportamiento del servidor/tiempo de ejecución.
...
Utilice `TALON_TUI_CONFIG` para cargar una ruta de configuración de TUI personalizada.
...
- `sound_pack`: ID del sound pack que se usará. El valor predeterminado es `talon.default`.
