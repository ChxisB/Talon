---
title: Servidor
description: Interactuar con el servidor talon a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `talon serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente talon puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `talon.local` |
...
Configure `TALON_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `talon`, o configure `TALON_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `talon serve` como a `talon web`.
...
Cuando ejecuta `talon`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor talon para interactuar con talon mediante programación.
:::

Esta arquitectura permite que talon admita múltiples clientes y le permite interactuar con talon mediante programación.

Puede ejecutar `talon serve` para iniciar un servidor independiente. Si tienes el
talon TUI ejecutándose, `talon serve` iniciará un nuevo servidor.
...
El servidor talon expone las siguientes API.
