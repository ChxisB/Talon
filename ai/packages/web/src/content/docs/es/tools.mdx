...
:::note
Esta herramienta solo está disponible cuando `TALON_EXPERIMENTAL_LSP_TOOL=true` (o `TALON_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCode o cuando la variable de entorno `TALON_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).
