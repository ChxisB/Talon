...
:::caution
Si no se establece `TALON_SERVER_PASSWORD`, el servidor no será seguro. Esto está bien para uso local, pero debe configurarse para acceso a la red.
:::

:::tip[Windows Usuarios]
Para obtener la mejor experiencia, ejecute `talon web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
Esto establece automáticamente el nombre de host en `0.0.0.0` y anuncia el servidor como `talon.local`.
...
Para proteger el acceso, establezca una contraseña utilizando la variable de entorno `TALON_SERVER_PASSWORD`:

```bash
TALON_SERVER_PASSWORD=secret talon web
```

El nombre de usuario predeterminado es `talon` pero se puede cambiar con `TALON_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `talon.json`:
