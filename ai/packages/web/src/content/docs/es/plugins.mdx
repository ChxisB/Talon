...
- `.talon/plugins/` - Complementos a nivel de proyecto
- `~/.config/talon/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/talon/node_modules/`.
...
1. Configuración global (`~/.config/talon/talon.json`)
2. Configuración del proyecto (`talon.json`)
3. Directorio global de complementos (`~/.config/talon/plugins/`)
4. Directorio de complementos del proyecto (`.talon/plugins/`)
...
- `client`: Un cliente SDK talon para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar talon.
...
Evite que talon lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a talon:
...
El ayudante `tool` crea una herramienta personalizada a la que talon puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para talon junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://talon.ai/docs/sdk) para obtener más detalles.
