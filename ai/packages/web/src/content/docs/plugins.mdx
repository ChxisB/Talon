...
- `.talon/plugins/` - Project-level plugins
- `~/.config/talon/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/talon/node_modules/`.
...
1. Global config (`~/.config/talon/talon.json`)
2. Project config (`talon.json`)
3. Global plugin directory (`~/.config/talon/plugins/`)
4. Project plugin directory (`.talon/plugins/`)
...
- `client`: An talon SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend talon.
...
Prevent talon from reading `.env` files:
...
Plugins can also add custom tools to talon:
...
The `tool` helper creates a custom tool that talon can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to talon alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://talon.ai/docs/sdk) for details.
