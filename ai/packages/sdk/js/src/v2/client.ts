...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-talon-directory", "directory"],
    ["x-talon-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-talon-directory")
  next.headers.delete("x-talon-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-talon-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-talon-workspace": config.experimental_workspaceID,
    }
  }
