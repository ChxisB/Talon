...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve talon route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "talon")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "talon")).toBe("openai")
    expect(statProvider("big-pickle", "", "talon")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "talon"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep talon as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "talon"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "talon"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep talon or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "talon"), country: "US" })).toMatchObject([
