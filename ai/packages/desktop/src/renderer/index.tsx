...
import type { UpdaterState } from "@talon-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@talon-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.TALON_CHANNEL === "prod" &&
...
const deepLinkEvent = "talon:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__TALON__ ??= {}
  const pending = window.__TALON__.deepLinks ?? []
  window.__TALON__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://talon.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("talon.global.dat").getItem("language")
