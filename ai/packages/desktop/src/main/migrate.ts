...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.talon.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `talon.settings.dat` is special: it maps to the `talon.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"talon.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // talon.settings.dat → the electron settings store ("talon.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "talon.global.dat").
  const storeName = filename === "talon.settings.dat" ? "talon.settings" : filename
