...
export function createWslServersController(
  ...
  const checkOpencode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    ...
    const version = resolved
      ...
      : null
    return talonCheck(distro, resolved, version, appVersion)
  }
  ...
  const refreshOpencodeCheckBackground = (id: string, distro: string) => {
    void checkOpencode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl talon check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkOpencode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl talon check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async probeOpencode(name: string) {
      await runJob({ kind: "probe-talon", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshOpencodeCheck(name, { signal: abort.signal })
      })
    },

    async installOpencode(name: string) {
      await runJob({ kind: "install-talon", distro: name, startedAt: Date.now() }, async (abort) => {
        ...
        await refreshOpencodeCheck(name, { signal: abort.signal })
        expectOpencodeVersion(state.talonChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.talonChecks, distro) : {}),
      })
    },
...
function talonCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "talon is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "talon is installed but could not run",
    }
  }
  return {
    distro,
