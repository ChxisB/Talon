...
export async function spawnWslSidecar(
  ...
  const talon = await resolveWslOpencode(distro)
  if (!talon) throw new Error(`OpenCode is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "talon"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export TALON_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export TALON_CLIENT=desktop",
    `export TALON_SERVER_USERNAME=${shellEscape(username)}`,
    `export TALON_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(talon)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
