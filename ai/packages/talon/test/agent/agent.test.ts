...
it.instance("plan agent denies edits except .talon/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".talon/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".talon", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.TALON_TEST_HOME
      process.env.TALON_TEST_HOME = test.directory
