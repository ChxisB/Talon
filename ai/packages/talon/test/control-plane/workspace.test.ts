...
import { EventSequenceTable } from "@talon-ai/core/event/sql"
...
import { WorkspaceV2 } from "@talon-ai/core/workspace"
import { WorkspaceTable } from "@talon-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@talon-ai/core/ripgrep"

const originalEnv = {
  TALON_AUTH_CONTENT: process.env.TALON_AUTH_CONTENT,
  TALON_EXPERIMENTAL_WORKSPACES: process.env.TALON_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.TALON_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@talon.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.TALON_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=talon-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.TALON_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.TALON_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.TALON_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=talon-test")
