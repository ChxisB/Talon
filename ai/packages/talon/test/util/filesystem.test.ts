...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "talon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "talon.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "talon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "talon.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["talon.json", "talon.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "talon.json"),
        path.join(project, "talon.jsonc"),
      ])
    })
  })
