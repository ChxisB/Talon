...
import { HttpRecorder } from "@talon-ai/http-recorder"
import { HttpRecorderInternal } from "@talon-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@talon-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@talon-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.talon.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.TALON_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["talon", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("TALON_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("TALON_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["talon", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["talon", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.TALON_RECORD_CONSOLE_TOKEN && process.env.TALON_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.talon,
        name: "OpenCode Zen",
        ...
        model,
        options: {
          apiKey: process.env.TALON_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.TALON_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["talon", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("TALON_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("TALON_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("TALON_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set TALON_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the talon LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "talon.json"),
      JSON.stringify({ $schema: "https://talon.ai/config.json", ...scenario.config(model) }),
    ),
  )
