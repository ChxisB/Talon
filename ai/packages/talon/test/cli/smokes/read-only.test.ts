...
describe("talon read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // TALON_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["mcp", "list"])
        talon.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["providers", "list"])
        talon.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["models"])
        talon.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["agent", "list"])
        talon.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh TALON_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["session", "list"])
        talon.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["stats"])
        talon.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under TALON_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ talon }) =>
      Effect.gen(function* () {
        const r = yield* talon.spawn(["db", "path"])
        talon.expectExit(r, 0, "db path")
