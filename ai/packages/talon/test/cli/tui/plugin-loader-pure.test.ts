...
`,
  ...
  const pure = process.env.TALON_PURE
  const meta = process.env.TALON_PLUGIN_META_FILE
  process.env.TALON_PURE = "1"
  process.env.TALON_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.TALON_PURE
    } else {
      process.env.TALON_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.TALON_PLUGIN_META_FILE
    } else {
      process.env.TALON_PLUGIN_META_FILE = meta
    }
  }
})
