...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@talon-ai/tui/context/editor"
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.TALON_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.TALON_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with TALON_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.TALON_EDITOR_SSE_PORT = "4020"
