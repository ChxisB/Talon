...
describe("talon acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ talon }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ talon }))
  ...
  cliIt.live(
    ...
    ({ talon }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ talon })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("talon-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "talon-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.TALON_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ talon }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("talon-login")
