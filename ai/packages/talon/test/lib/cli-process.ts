// Subprocess test harness for the talon CLI. Spawns the real binary against
...
//
// Configuration flows through talon's built-in test affordances:
//   - TALON_CONFIG_CONTENT      : provider config inline, no files to find
//   - TALON_TEST_HOME           : pins os.homedir() → tmpdir
//   - TALON_DISABLE_PROJECT_CONFIG : skip walking up for talon.json
//   - TALON_PURE                : skip external plugin discovery + install
//   - TALON_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
const talonRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(talonRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    TALON_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    TALON_CONFIG_CONTENT: configJson,
    TALON_DISABLE_PROJECT_CONFIG: "1",
    TALON_PURE: "1",
    TALON_DISABLE_AUTOUPDATE: "1",
    TALON_DISABLE_AUTOCOMPACT: "1",
    TALON_DISABLE_MODELS_FETCH: "1",
    TALON_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `talon run`. New flags should
...
// `talon serve` is a long-lived process — it never exits on its own.
...
// `talon acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `talon acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("talon.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("talon.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "talon server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `talon serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("talon.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    // and hit endpoints on `talon.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "talon") {
...
// every call site. Use this for any test that needs the talon CLI fixture.
//
...
// resources (e.g. `talon.serve`) without an extra `Effect.scoped` wrapper —
