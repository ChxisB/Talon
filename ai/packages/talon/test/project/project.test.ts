...
import { ProjectDirectories } from "@talon-ai/core/project/directories"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const talonFile = path.join(tmp, ".git", "talon")
      expect(yield* Effect.promise(() => Bun.file(talonFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "talon")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "talon")
      const wrongCache = path.join(parentDir, ".git", "talon")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "talon")
      const cacheB = path.join(bareB, "talon")
      const wrongCache = path.join(parentDir, ".git", "talon")
