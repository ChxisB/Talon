...
- Author package entrypoint is `@talon-ai/plugin/tui`.
...
```json
{
  "$schema": "https://talon.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/talon-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@talon-ai/plugin/tui`.
- `@talon-ai/plugin` exports `./tui` and declares optional peer deps on `@tui/core` and `@tui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @tui/solid */
import type { TuiPlugin, TuiPluginModule } from "@talon-ai/plugin/tui"
...
- If `engines.talon` is absent, no check is performed (backward compatible).
...
- `talon plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `talon plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.talon` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.talon`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.talon`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.talon` dir or `TALON_CONFIG_DIR`, OpenCode installs `@talon-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@talon-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@talon-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"talon"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `talon.default`.
...
- Local plugins persist installed themes under the local `.talon/themes` area near the plugin config source.
...
- `--pure` / `TALON_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.talon/plugins/tui-smoke.tsx`
- Local vim plugin: `.talon/plugins/tui-vim.tsx`
- Local smoke config: `.talon/tui.json`
- Local smoke theme: `.talon/plugins/smoke-theme.json`
