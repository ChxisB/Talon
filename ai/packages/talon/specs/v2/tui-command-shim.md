...
Delete `packages/talon/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/talon/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/talon/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/talon`
- TUI plugin loader tests in `packages/talon` if runtime plugin API wiring changed
