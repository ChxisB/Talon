...
Practical reference for a future `packages/server` split after the talon
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/talon`.
...
Until enough shared service code lives outside `packages/talon`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/talon`

It should not import `packages/talon` services while `packages/talon`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on talon-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/talon` implementation details.
