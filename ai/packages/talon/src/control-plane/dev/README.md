This is a plugin to simulate a remote environment locally. Add this to `.talon/talon.jsonc`:

```json
  "plugin": ["../packages/talon/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/talon/script/run-workspace-server
```
