...
import type { LLMClientShape } from "@talon-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("talon"))
    return { type: "unsupported", reason: "provider is not openai, talon, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @talon-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains talon-owned. The native runtime only adapts
      // the @talon-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
