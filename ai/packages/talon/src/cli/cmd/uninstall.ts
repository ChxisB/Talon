...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall talon and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g talon-ai",
      pnpm: "pnpm uninstall -g talon-ai",
      bun: "bun remove -g talon-ai",
      yarn: "yarn global remove talon-ai",
      brew: "brew uninstall talon",
      choco: "choco uninstall talon",
      scoop: "scoop uninstall talon",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "talon", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# talon") || content.includes(".talon/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".talon/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".talon/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".talon"))
    ) {
      continue
    }
