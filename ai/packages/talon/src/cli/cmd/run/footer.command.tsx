...
import { TextAttributes, type InputRenderable, type KeyEvent } from "@tui/core"
import { useKeyboard, type JSX } from "@tui/solid"
...
export function RunModelSelectBody(props: {
  ...
  const entries = createMemo<ModelEntry[]>(() =>
    (props.providers() ?? [])
        ...
        Object.entries(provider.models)
          ...
          .map(([modelID, model]) => {
            ...
            const footer = current
              ? "current"
              : model.cost?.input === 0 && provider.id === "talon"
                ? "Free"
                : title !== modelID
                  ? modelID
                  : undefined
      ...
      .sort((a, b) => {
        const provider = Number(a.providerID !== "talon") - Number(b.providerID !== "talon")
        if (provider !== 0) {
          return provider
        }
