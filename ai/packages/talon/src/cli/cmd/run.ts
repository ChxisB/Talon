import type { PermissionV1 } from "@talon-ai/core/v1/permission"
// CLI entry point for `talon run`.
//
// Handles three modes:
...
//      talon server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run talon with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running talon server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to TALON_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to TALON_SERVER_USERNAME or 'talon')",
      })
      .option("dir", {
        type: "string",
