...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/talon"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "talon")
    default:
      return "/etc/talon"
  }
}
...
export function managedConfigDir() {
  return process.env.TALON_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
