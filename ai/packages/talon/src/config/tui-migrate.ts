...
const TUI_SCHEMA_URL = "https://talon.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from talon.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const talon = await talonFiles(input)
  for (const file of talon) {
...
async function talonFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "talon"),
    ...(await Filesystem.findUp(["talon.json", "talon.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "talon"))
  }
  if (Flag.TALON_CONFIG) files.push(Flag.TALON_CONFIG)
