...
import { isRecord } from "@talon-ai/tui/util/record"
...
import { TuiKeybind } from "@talon-ai/tui/config/keybind"
...
import { TuiConfig } from "@talon-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@talon/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old talon.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.talon`
  // folders between cwd and home, and TALON_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.TALON_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit TALON_TUI_CONFIG override, if set.
  if (Flag.TALON_TUI_CONFIG) {
    const configFile = Flag.TALON_TUI_CONFIG
  ...
  // 4. `.talon` directories (and TALON_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".talon") || dir === Flag.TALON_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".talon") && dir !== Flag.TALON_CONFIG_DIR) continue
