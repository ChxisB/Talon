...
export const DEPRECATED_PLUGIN_PACKAGES = ["talon-openai-codex-auth", "talon-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, talonVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(talonVersion) || semver.major(talonVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.talon
  if (typeof range !== "string") return
  if (!semver.satisfies(talonVersion, range)) {
    throw new Error(`Plugin requires talon ${range} but running ${talonVersion}`)
  }
}
