import { runtimeModules as keymapRuntimeModules } from "@tui/keymap/runtime-modules"
...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@talon-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@talon-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@talon-ai/tui/context/theme"
import { Global } from "@talon-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@talon-ai/core/util/flock"
import { Flag } from "@talon-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@talon-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@talon-ai/core/v1/config/plugin"
import { createCommandShim } from "@talon-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@talon-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".talon"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".talon", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".talon", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.TALON_PURE ? [] : pluginOrigins
    if (Flag.TALON_PURE && pluginOrigins.length) {
    }
