...
import type { Message, OpencodeClient, SessionMessageResponse } from "@talon-ai/sdk/v2"
...
export const AuthMethodID = "talon-login"
...
export class Service extends Context.Service<Service, Interface>()("@talon/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `talon auth login` in the terminal",
      name: "Login with talon",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, talon provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const talonProvider = providers[ProviderV2.ID.make("talon")]
  const talonModel = talonProvider ? Provider.sort(Object.values(talonProvider.models))[0] : undefined
  if (talonProvider && talonModel) return { providerID: talonProvider.id, modelID: talonModel.id }
