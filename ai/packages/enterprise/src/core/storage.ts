...
import { lazy } from "@talon-ai/core/util/lazy"
...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.TALON_STORAGE_BUCKET!
    const region = process.env.TALON_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.TALON_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.TALON_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.TALON_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.TALON_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.TALON_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://${accountId}.r2.cloudflarestorage.com`, process.env.TALON_STORAGE_BUCKET!)
  }

  const adapter = lazy(() => {
    const type = process.env.TALON_STORAGE_ADAPTER
