...
export class ConfigService extends Context.Service<ConfigService, Config>()("@talon/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.TALON_WEBSEARCH_PROVIDER === "exa" || process.env.TALON_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.TALON_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("TALON_EXPERIMENTAL") || truthy("TALON_ENABLE_EXA") || truthy("TALON_EXPERIMENTAL_EXA"),
    enableParallel: truthy("TALON_ENABLE_PARALLEL") || truthy("TALON_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `talon/${InstallationVersion}`,
