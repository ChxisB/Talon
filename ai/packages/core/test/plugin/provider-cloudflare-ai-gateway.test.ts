...
import { CloudflareAIGatewayPlugin } from "@talon-ai/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              ...
              metadata: { invoked_by: "test", project: "talon" },
              cacheTtl: 300,
              cacheKey: "cache-key",
              skipCache: true,
              collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("talon/"),
            },
          },
        })
      }),
    ...
    withEnv(cloudflareEnv(), () =>
      Effect.gen(function* () {
        ...
        yield* plugin.trigger(
          "aisdk.sdk",
          {
            ...
            options: {
              name: "cloudflare-ai-gateway",
              headers: {
                "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "talon" }),
              },
            },
          },
          {},
        ...
        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "talon" },
        })
      }),
    ),
  )
