...
describe("OpencodePlugin", () => {
    ...
    withEnv({ TALON_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* transform((catalog) => {
          const item = provider("talon")
          catalog.provider.update(item.id, () => {})
          const paid = model("talon", "paid", { cost: cost(1) })
          catalog.model.update(item.id, paid.id, (draft) => {
            draft.cost = [...paid.cost]
          })
        })
        expect((yield* catalog.provider.get(ProviderV2.ID.talon)).request.body.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.talon, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
    ...
    withEnv({ TALON_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* transform((catalog) => {
          const item = provider("talon")
          catalog.provider.update(item.id, () => {})
          const free = model("talon", "free", { cost: cost(0) })
          catalog.model.update(item.id, free.id, (draft) => {
            draft.cost = [...free.cost]
          })
        })
        expect((yield* catalog.provider.get(ProviderV2.ID.talon)).request.body.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.talon, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
    ...
    withEnv({ TALON_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* transform((catalog) => {
          const item = provider("talon")
          catalog.provider.update(item.id, () => {})
          const outputOnly = model("talon", "output-only", { cost: cost(0, 1) })
          catalog.model.update(item.id, outputOnly.id, (draft) => {
            draft.cost = [...outputOnly.cost]
          })
        })
        expect((yield* catalog.provider.get(ProviderV2.ID.talon)).request.body.apiKey).toBe("public")
        expect((yield* catalog.model.get(ProviderV2.ID.talon, ModelV2.ID.make("output-only"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses TALON_API_KEY as credentials", () =>
    withEnv({ TALON_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        yield* plugin.add(pluginWithIntegrations(yield* Integration.Service))
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.talon)).request.body.apiKey).toBeUndefined()
        expect((yield* catalog.model.get(ProviderV2.ID.talon, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ TALON_API_KEY: undefined, CUSTOM_TALON_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.update((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("talon"),
            method: { type: "env", names: ["CUSTOM_TALON_API_KEY"] },
          })
        })
        const transform = yield* catalog.transform()
        yield* transform((catalog) => {
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ TALON_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect((yield* catalog.provider.get(ProviderV2.ID.talon)).request.body.apiKey).toBe("configured")
        expect((yield* catalog.model.get(ProviderV2.ID.talon, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-talon providers and models", () =>
    withEnv({ TALON_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
  ...
  it.effect("prefers gpt-5-nano as the talon small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.talon
