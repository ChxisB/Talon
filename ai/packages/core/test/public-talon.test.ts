...
describe("public native OpenCode API", () => {
  it.effect("exposes only the intentional Session capabilities", () =>
    Effect.gen(function* () {
      ...
      expect(Object.keys(talon.sessions).sort()).toEqual([
        "context",
        "create",
        "events",
        "get",
      ...
      expect(yield* talon.sessions.list()).toBeArray()
      yield* talon.tools.register({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* talon.sessions.switchModel({ sessionID, model })

          expect((yield* talon.sessions.get(sessionID)).model).toEqual(model)
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap(([available, disabled]) =>
        Effect.gen(function* () {
          ...
          yield* talon.sessions.switchModel({ sessionID: availableID, model: ref({ variant: "default" }) })
          const disabledError = yield* talon.sessions
            .switchModel({ sessionID: disabledID, model: ref() })
            .pipe(Effect.flip)
          const missingError = yield* talon.sessions
          ...
          expect((yield* talon.sessions.get(availableID)).model).toEqual(ref({ variant: "default" }))
          expect((yield* talon.sessions.get(disabledID)).model).toBeUndefined()
        }),
      ),
    ),
  )
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          yield* talon.sessions.switchModel({ sessionID, model: selected })
          ...
          expect((yield* talon.sessions.get(sessionID)).model).toEqual(selected)
        }),
      ),
    ),
  )
