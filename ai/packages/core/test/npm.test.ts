...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@talon/acme")).toBe("@talon/acme")
    expect(Npm.sanitize("@talon/acme@1.0.0")).toBe("@talon/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/talon/acme.git"
    const expected = win ? "acme@git+https_//github.com/talon/acme.git" : spec
