...
import { Tag } from "@talon-ai/ui/v2/badge-v2"
import { ButtonV2 } from "@talon-ai/ui/v2/button-v2"
import { Dialog } from "@talon-ai/ui/v2/dialog-v2"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.talonChecks[item.config.distro]
          const talonAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-talon" && wsl.data.job.distro === item.config.distro
          return (
