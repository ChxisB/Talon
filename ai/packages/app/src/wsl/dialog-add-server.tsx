import { Button } from "@talon-ai/ui/button"
import { useDialog } from "@talon-ai/ui/context/dialog"
import { Spinner } from "@talon-ai/ui/spinner"
import { showToast } from "@talon-ai/ui/toast"
...
type WslServerStep = "wsl" | "distro" | "talon"

const STEPS: WslServerStep[] = ["wsl", "distro", "talon"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const talonCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.talonChecks[distro] ?? null
  })
  ...
  const talonReady = createMemo(() => {
    const check = talonCheck()
  ...
  const installingOpencode = createMemo(() => {
    ...
    return job?.kind === "install-talon" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && talonReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-talon"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.talonChecks[distro]) {
      return { key: `probe-talon:${distro}`, run: () => api.probeOpencode(distro) }
    }
    return null
  })
  ...
  const talonMessage = createMemo(() => {
    ...
    if (talonReady()) {
      return distro
        ? language.t("wsl.onboarding.talonReadyIn", { distro })
        : language.t("wsl.onboarding.talonReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.talon"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : talonCheck()?.matchesDesktop === false
                  ? "warning"
                  : talonReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "talon"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.talon")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                    ...
                    <Show when={!talonReady() || talonCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                ...
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{talonMessage()}</div>
                <Show when={talonCheck()?.matchesDesktop === false ? talonCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "talon" && allReady() && selectedDistro()}>
