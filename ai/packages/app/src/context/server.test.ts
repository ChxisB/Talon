  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://talon.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://talon.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://talon.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://talon.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://talon.example.com"),
      ),
    ).toEqual({
