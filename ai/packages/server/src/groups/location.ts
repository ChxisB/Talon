...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-talon-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-talon-directory"] ? decode(request.headers["x-talon-directory"]) : process.cwd())
