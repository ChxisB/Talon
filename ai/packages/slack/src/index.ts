...
import { createOpencode, type ToolPart } from "@talon-ai/sdk"
...
console.log("🚀 Starting talon server...")
const talon = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await talon.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new talon session...")
    const { client, server } = talon
    ...
    console.log("✅ Created talon session:", createResult.data.id)
  ...
  console.log("📝 Sending to talon:", message.text)
