...
import { createOpencode } from "@talon-ai/sdk"
...
`)
  ...
  const talon = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await talon.client.session.create()
    const result = await talon.client.session
      .prompt({
