...
const skip = new Set([".git", ".talon", ".turbo", "dist", "node_modules"])
const keys = ["@tui/core", "@tui/keymap", "@tui/solid"] as const
...
if (fixed.length > 0) {
  console.log("Removed stale @tui/spinner peer lockfile entries:")
...
async function fixKnownLockfileIssues() {
  ...
  if (stale.some((item) => !item.entry.startsWith("@tui/spinner/@tui/"))) return []

  const removed = txt
    .split("\n")
    .map((line) => line.match(/^    "(@tui/spinner\/@tui\/[^\"]+)": /)?.[1])
  ...
  await Bun.write(
    ...
    txt
      .split("\n")
      .filter((line) => !line.match(/^    "@tui/spinner\/@tui\//))
      .join("\n"),
  )
  return removed
}
...
function findStaleLockfileEntriesInText(txt: string) {
  return Array.from(txt.matchAll(/^    "([^"]+)": \["(@tui\/(?:core(?:-[^@"]+)?|keymap|solid))@([^"]+)"/gm))
    .map((match) => ({
      entry: match[1]!,
      pkg: match[2]!,
      version: match[3]!,
