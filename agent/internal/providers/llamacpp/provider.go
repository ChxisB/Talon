...
// Uses Anthropic Messages API at http://localhost:8080
package llamacpp
...
func metadata() providers.ProviderMetadata {
	return providers.ProviderMetadata{
		ID:              providerName,
		Name:            "llama.cpp",
		Description:     "llama.cpp server - Anthropic Messages API compatible",
		APIType:         "anthropic",
		BaseURL:         defaultBase,
		...
		DefaultModels:   []string{"llamacpp/local-model"},
		ModelPrefix:     "llamacpp/",
...
func New(cfg providers.ProviderConfig, _ *config.Settings) (providers.Provider, error) {
	...
	return &Provider{
		...
		transport: anthropic.NewTransport(anthropic.Config{
			Name:    providerName,
			BaseURL: baseURL,
			APIKey:  cfg.APIKey, // llama.cpp doesn't require API key, but accept if provided
		}),
	}, nil
}
