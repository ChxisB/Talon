...
function validatePackage(config: PackageConfig): void {
  ...
  // For packages that publish an @tui/core dependency, check the version.
  if (config.requiresCore) {
    const coreDependencyVersion = distPackageJson.dependencies?.["@tui/core"]
    if (coreDependencyVersion !== packageJson.version) {
      console.error(`ERROR: @tui/core dependency version mismatch in dist`)
      console.error(`  Expected: ${packageJson.version}`)
      console.error(`  Found: ${coreDependencyVersion}`)
      console.error("Please rebuild the package with 'bun run build'")
      process.exit(1)
    }
    console.log(`SUCCESS: @tui/core dependency version matches`)
  }
