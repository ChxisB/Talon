...
if [ -z "$TARGET_ROOT" ]; then
  ...
  echo "@tui/core, @tui/keymap, and @tui/qrcode are always linked."
  echo "Framework packages can be linked with the flags below."
  echo ""
  echo "Options:"
  echo "  --react    Also link @tui/react and React dependencies"
  echo "  --solid    Also link @tui/solid and solid-js"
  exit 1
fi
...
# Always link @tui/core
echo "Linking @tui/core..."
link_in_bun_cache "@tui+core@*" "@tui/core" "$OPENTUI_ROOT/packages/core"

# Always link @tui/keymap
echo "Linking @tui/keymap..."
link_in_bun_cache "@tui+keymap@*" "@tui/keymap" "$OPENTUI_ROOT/packages/keymap"

# Always link @tui/qrcode
echo "Linking @tui/qrcode..."
link_in_bun_cache "@tui+qrcode@*" "@tui/qrcode" "$OPENTUI_ROOT/packages/qrcode"
...
# Link @tui/solid if requested
if [ "$LINK_SOLID" = true ]; then
  echo "Linking @tui/solid..."
  link_in_bun_cache "@tui+solid@*" "@tui/solid" "$OPENTUI_ROOT/packages/solid"
...
# Link @tui/react if requested
if [ "$LINK_REACT" = true ]; then
  echo "Linking @tui/react..."
  link_in_bun_cache "@tui+react@*" "@tui/react" "$OPENTUI_ROOT/packages/react"
