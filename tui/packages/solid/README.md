...
```bash
bun install solid-js @tui/solid
```
...
```ts
import solidPlugin from "@tui/solid/bun-plugin"
...
```tsx
import { testRender } from "@tui/solid"
...
QR code support is available from `@tui/qrcode/solid` and must be registered explicitly with `registerQRCode()`.
