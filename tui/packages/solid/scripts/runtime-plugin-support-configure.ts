...
import * as solidRuntime from "@tui/solid"
...
const defaultRuntimeModules: Record<string, RuntimeModuleEntry> = {
  "@tui/solid": solidRuntime as Record<string, unknown>,
...
function assertCompatibleInstall(
  ...
  for (const specifier of Object.keys(modules)) {
    if (!install.specifiers.has(specifier)) {
      throw new Error(
        `OpenTUI Solid runtime plugin support is already installed without ${specifier}. Call ensureRuntimePluginSupport({ additional }) from @tui/solid/runtime-plugin-support/configure before importing @tui/solid/runtime-plugin-support.`,
      )
    }
  }
...
export function ensureRuntimePluginSupport(options: SolidRuntimePluginSupportOptions = {}): boolean {
  ...
  ensureSolidTransformPlugin({
    moduleName: runtimeModuleIdForSpecifier("@tui/solid"),
