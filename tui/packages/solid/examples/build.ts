...
const workspaceAliasPlugin: BunPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@tui\/solid$/ }, () => {
      return {
    ...
    build.onResolve({ filter: /^@tui\/three\/runtime-modules$/ }, () => {
      return {
        path: join(packageRoot, "..", "three", "src", "runtime-modules.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/solid\/runtime-plugin-support$/ }, () => {
      return {
        path: join(packageRoot, "scripts", "runtime-plugin-support.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/solid\/runtime-plugin-support\/configure$/ }, () => {
      return {
