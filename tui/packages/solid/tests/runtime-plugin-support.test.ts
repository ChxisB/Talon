...
describe("solid runtime plugin support", () => {
  ...
  it("throws when modules are added after side-effect installation", () => {
    ...
    expect(result.exitCode).toBe(0)
    expect(stdout).toContain("OpenTUI Solid runtime plugin support is already installed without @tui/keymap")
    expect(stdout).toContain("@tui/solid/runtime-plugin-support/configure")
  })
})
