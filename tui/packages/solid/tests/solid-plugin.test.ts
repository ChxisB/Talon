...
import { runtimeModuleIdForSpecifier } from "@tui/core/runtime-plugin"
...
describe("solid transform plugin", () => {
  ...
  it("uses @tui/solid as the default JSX runtime module", async () => {
    ...
    try {
      ...
      expect(transformed.contents).toContain("@tui/solid")
  ...
  it("applies custom module resolver rewrites when configured", async () => {
    const runtimeSolidModule = runtimeModuleIdForSpecifier("@tui/solid")
    const runtimeCoreModule = runtimeModuleIdForSpecifier("@tui/core")
    const runtimeFixtureModule = runtimeModuleIdForSpecifier("fixture-sync")

    const tempFile = createTempTsxFile(
      [
        'import { engine } from "@tui/core"',
    ...
    try {
      ...
      expect(transformed.contents).not.toContain('from "@tui/core"')
