...
`@tui/keymap/addons` ships ready-made building blocks:
...
`@tui/keymap/addons/opentui` adds OpenTUI-specific pieces: layout-independent matching via `event.baseCode`, and pre-wired textarea / edit-buffer commands. Direct event matches win before fallback matches, even across layers.

## Entry Points

- `@tui/keymap` — core API
- `@tui/keymap/addons` — universal addons
- `@tui/keymap/addons/opentui` — universal + OpenTUI addons
- `@tui/keymap/extras/graph` — graph snapshot helpers for debug and graph UIs
- `@tui/keymap/testing` — host-agnostic fake keymap host and diagnostics for addon tests
- `@tui/keymap/html` — HTML adapter
- `@tui/keymap/opentui` — OpenTUI adapter
- `@tui/keymap/react` — `KeymapProvider`, `useKeymap`, `useBindings`, `useActiveKeys`, `usePendingSequence`, `reactiveMatcherFromStore`
- `@tui/keymap/solid` — `KeymapProvider`, `useKeymap`, `useKeymapSelector`, `useBindings`, `reactiveMatcherFromSignal`
- `@tui/keymap/extras` — helpers for cheat-sheet UIs (`createBindingLookup`, `commandBindings`, `formatCommandBindings`)

## Usage

```tsx
import { registerDefaultKeys } from "@tui/keymap/addons"
import { createOpenTuiKeymap } from "@tui/keymap/opentui"
import { KeymapProvider } from "@tui/keymap/react"
