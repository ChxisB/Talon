...
- `--react` - Also link `@tui/react` and React dependencies
- `--solid` - Also link `@tui/solid` and SolidJS dependencies
- `--dist` - Link built `dist` directories instead of source
- `--copy` - Copy instead of symlink (requires `--dist`)
- `--subdeps` - Find and link packages that depend on opentui (e.g., `@tui/spinner`)
...
- Main packages: `@tui/core`, `@tui/solid`, `@tui/react`
- Peer dependencies: `solid-js`, `react`, `react-dom`, `react-reconciler`
- Subdependencies (with `--subdeps`): Packages like `@tui/spinner` that depend on opentui
