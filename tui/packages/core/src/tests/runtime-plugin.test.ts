...
describe("runtime plugin", () => {
  it("registers core runtime modules by default", async () => {
    ...
    const coreResolution = await resolveSpecifier(resolveHandlers, "@tui/core")
    const threeResolution = await resolveSpecifier(resolveHandlers, "@tui/three")
    const coreTestingResolution = await resolveSpecifier(resolveHandlers, "@tui/core/testing")

    expect(coreResolution).toEqual({ path: runtimeModuleIdForSpecifier("@tui/core") })
    expect(threeResolution).toBeUndefined()
    expect(coreTestingResolution).toEqual({ path: runtimeModuleIdForSpecifier("@tui/core/testing") })
  ...
  it("registers @tui/three only when added explicitly", async () => {
    const { build, resolveHandlers, modules } = createMockBuild()

    createRuntimePlugin({
      additional: {
        "@tui/three": { ThreeRenderable: "three-value" },
      },
    }).setup(build as any)

    const threeResolution = await resolveSpecifier(resolveHandlers, "@tui/three")

    expect(threeResolution).toEqual({ path: runtimeModuleIdForSpecifier("@tui/three") })

    if (!threeResolution) {
      throw new Error("Expected @tui/three runtime module resolution")
    }
    ...
    if (!threeModuleFactory) {
      throw new Error("Expected @tui/three runtime module factory")
    }
  ...
  it("encodes runtime module IDs deterministically", () => {
    expect(runtimeModuleIdForSpecifier("@tui/core/testing")).toBe(
