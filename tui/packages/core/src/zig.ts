...
async function resolveNativePackage() {
  if (process.platform === "darwin") {
    ...
    if (process.arch === "x64") return await import("@tui/core-darwin-x64")
    // @ts-ignore Optional native package may be absent when building on another platform.
    if (process.arch === "arm64") return await import("@tui/core-darwin-arm64")
  }
  ...
  if (process.platform === "linux") {
    ...
    if (process.arch === "x64") {
      if (process.env.OPENTUI_LIBC === "musl") {
        ...
        return await import("@tui/core-linux-x64-musl")
      } else {
        // @ts-ignore Optional native package may be absent when building on another platform.
        return await import("@tui/core-linux-x64")
      }
    }
    ...
    if (process.arch === "arm64") {
      if (process.env.OPENTUI_LIBC === "musl") {
        // @ts-ignore Optional native package may be absent unless building a musl target.
        return await import("@tui/core-linux-arm64-musl")
      } else {
        // @ts-ignore Optional native package may be absent when building on another platform.
        return await import("@tui/core-linux-arm64")
      }
    }
  }
  ...
  if (process.platform === "win32") {
    // @ts-ignore Optional native package may be absent when building on another platform.
    if (process.arch === "x64") return await import("@tui/core-win32-x64")
    // @ts-ignore Optional native package may be absent when building on another platform.
    if (process.arch === "arm64") return await import("@tui/core-win32-arm64")
  }
...
} else if (!existsSync(targetLibPath)) {
  throw new Error(`opentui is not supported on the current platform: ${process.platform}-${process.arch}`)
}
