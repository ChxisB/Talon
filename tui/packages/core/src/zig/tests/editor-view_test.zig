...
test "EditorView - horizontal scroll: combined vertical and horizontal scrolling" {
    ...
    defer text_list.deinit(std.testing.allocator);
    try text_list.appendSlice(std.testing.allocator, line0);
    var i: u32 = 1;
    while (i < 20) : (i += 1) {
        try text_list.appendSlice(std.testing.allocator, repeated_line);
    }
