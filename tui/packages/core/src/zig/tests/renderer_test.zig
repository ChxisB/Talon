...
const Timer = struct {
    ...
    fn start() Timer {
        ...
        return .{ .started = @as(i128, ts.sec) * 1_000_000_000 + ts.nsec };
    }
    fn read(t: *const Timer) u64 {
        var ts: std.c.timespec = undefined;
        _ = std.c.clock_gettime(@as(std.c.clockid_t, @enumFromInt(1)), &ts);
        const now = @as(i128, ts.sec) * 1_000_000_000 + ts.nsec;
        return @as(u64, @intCast(now - t.started));
    }
};
...
const SlowThreadSafeOutput = struct {
    ...
    fn write(ctx: *anyopaque, data: []const u8) void {
        ...
        var ts: std.c.timespec = .{ .sec = 0, .nsec = @intCast(self.delay_ns) };
...
test "renderer - buffered debug dump includes non-threaded last render" {
    ...
    var dump_writer = std.Io.Writer.fixed(&dump_buf);
    cli_renderer.backend.dumpTo(&dump_writer);
    const dump = dump_writer.buffer[0..dump_writer.end];
...
test "threaded buffered backend skips instead of blocking behind output" {
    ...
    backend.beginFrame();
    {
        ...
        try w.writeAll("large graphics frame");
    }
    ...
    {
        var w = backend.writer();
        try w.writeAll("next graphics frame");
    }
    try std.testing.expectEqual(@import("../renderer-output.zig").WriteStatus.ok, backend.endFrame());
    backend.setUseThread(false);
