...
const Mutex = struct {
    ...
    fn tryLock(m: *Mutex) bool {
        return m.inner.swap(1, .acquire) == 0;
    }
...
const Condition = struct {
    ...
    fn wait(c: *Condition, m: *Mutex) void {
        const epoch = c.wakeups.load(.acquire);
        m.unlock();
        while (c.wakeups.load(.acquire) == epoch) {
            std.Thread.yield() catch {};
        }
        m.lock();
    }

    fn signal(c: *Condition) void {
        _ = c.wakeups.fetchAdd(1, .release);
    }

    fn broadcast(c: *Condition) void {
        _ = c.wakeups.fetchAdd(1, .release);
    }
};
...
pub const BufferedBackend = struct {
    ...
    pub const Writer = struct {
        ...
        pub fn writeAll(self: *Writer, data: []const u8) error{BufferFull}!void {
            const written = try bufferWrite(self.ctx, data);
        ...
        pub fn print(self: *Writer, comptime fmt: []const u8, args: anytype) error{BufferFull}!void {
            var buf: [4096]u8 = undefined;
            const msg = std.fmt.bufPrint(&buf, fmt, args) catch return error.BufferFull;
    ...
    pub fn endFrame(self: *BufferedBackend) WriteStatus {
        ...
        const writeStart = microTimestamp();
        ...
        if (self.useThread) {
            ...
            self.lastWriteTimeUs = @as(f64, @floatFromInt(microTimestamp() - writeStart));
        }
...
pub const FeedBackend = struct {
    ...
    pub const Writer = struct {
        ...
        pub fn writeAll(self: *Writer, data: []const u8) error{BufferFull}!void {
            const written = try frameWrite(self.ctx, data);
            if (written != data.len) return error.BufferFull;
        }
