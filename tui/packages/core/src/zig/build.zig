...
fn nativeExecutableTarget(b: *std.Build) std.Build.ResolvedTarget {
    ...
    // Zig's ELF linker currently fails on newer glibc startup objects
...
fn pathExists(b: *std.Build, path: []const u8) bool {
    ...
    std.Io.Dir.access(dir, b.graph.io, path, .{}) catch return false;
    return true;
}
...
pub fn build(b: *std.Build) void {
    ...
    addNativeAudioDependencies(b, bench_ffi_mod, native_target, macos_sdk_path);
