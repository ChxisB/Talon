...
pub const UnifiedTextBuffer = struct {
    ...
    pub fn loadFile(self: *Self, path: []const u8) TextBufferError!void {
        ...
        while (true) {
            const bytes_read = std.posix.read(fd, &buf) catch return TextBufferError.OutOfMemory;
            if (bytes_read == 0) break;
            content.appendSlice(self.allocator, buf[0..bytes_read]) catch return TextBufferError.OutOfMemory;
        }

        const text = self.allocator.dupe(u8, content.items) catch return TextBufferError.OutOfMemory;
        const mem_id = try self.mem_registry.register(text, false);
        try self.setTextInternal(mem_id, text);
    }
