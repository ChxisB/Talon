...
fn writePassthroughSequence(self: *Terminal, tty: anytype, sequence: []const u8) !void {
    ...
    if (!self.remote) {
        var env_map_storage: ?std.process.Environ.Map = null;
        const env_map: ?*const std.process.Environ.Map = self.opts.env_map orelse blk: {
            var map = std.process.Environ.Map.init(std.heap.page_allocator);
        {
                var i: usize = 0;
                while (std.c.environ[i]) |entry| {
                    const s = std.mem.sliceTo(entry, 0);
                    if (std.mem.indexOfScalar(u8, s, '=')) |eq| {
                        map.put(s[0..eq], s[eq + 1 ..]) catch break;
                    }
                    i += 1;
                }
            }
...
pub fn writeNotification(self: *Terminal, allocator: std.mem.Allocator, tty: anytype, message: []const u8, title: ?[]const u8) !bool {
    ...
    try self.writePassthroughSequence(tty, writer.buffered());
    return true;
}
