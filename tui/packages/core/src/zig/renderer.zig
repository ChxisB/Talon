...
pub const CliRenderer = struct {
    ...
    pub fn setupTerminal(self: *CliRenderer, useAlternateScreen: bool) void {
        ...
        var w: std.Io.Writer = .fixed(&queryBuf);
        self.terminal.queryTerminalSend(&w) catch {
            logger.warn("Failed to query terminal capabilities", .{});
        };
        self.backend.writeOut(w.buffered());
    ...
    fn setupTerminalWithoutDetection(self: *CliRenderer, useAlternateScreen: bool, reserve_non_alt_surface: bool) void {
        ...
        var w: std.Io.Writer = .fixed(&setupBuf);

        w.writeAll(ansi.ANSI.saveCursorState) catch {};

        if (useAlternateScreen) {
            self.terminal.enterAltScreen(&w) catch {};
        } else if (reserve_non_alt_surface) {
            ansi.ANSI.makeRoomForRendererOutput(&w, @max(self.height, 1)) catch {};
        }

        self.terminal.setCursorPosition(1, 1, false);
        const useKitty = self.terminal.opts.kitty_keyboard_flags > 0;
        self.terminal.enableDetectedFeatures(&w, useKitty) catch {};
    ...
    pub fn performShutdownSequence(self: *CliRenderer) void {
        ...
        var w: std.Io.Writer = .fixed(&shutdownBuf);

        self.terminal.resetState(&w) catch {
        ...
        if (self.useAlternateScreen) {
            ...
            w.writeAll("\x1b[H\x1b[J") catch {};
        } else if (self.clearOnShutdown and self.renderOffset > 0) {
            self.clearSplitFooterSurface(&w);
        }

        w.writeAll(ansi.ANSI.resetCursorColorFallback) catch {};
        w.writeAll(ansi.ANSI.resetCursorColor) catch {};
        w.writeAll(ansi.ANSI.defaultCursorStyle) catch {};
        ...
        var ts: std.c.timespec = .{ .sec = 0, .nsec = @intCast(10 * std.time.ns_per_ms) };
        _ = std.c.nanosleep(&ts, null);
        self.backend.writeOut(ansi.ANSI.showCursor);
        var ts2: std.c.timespec = .{ .sec = 0, .nsec = @intCast(10 * std.time.ns_per_ms) };
    ...
    pub fn setRenderOffset(self: *CliRenderer, offset: u32) void {
        if (self.terminalSetup and !self.useAlternateScreen and self.renderOffset > 0 and offset == 0) {
            ...
            var w: std.Io.Writer = .fixed(&clearBuf);
            self.clearSplitFooterSurface(&w);
        self.backend.writeOut(w.buffered());
        }
    ...
    pub fn prepareRenderFrameWithWriter(self: *CliRenderer, writer: anytype, force: bool, sync_started: bool) void {
        const renderStartTime = microTimestamp();
        ...
        const renderEndTime = microTimestamp();
    ...
    fn fileWrite(fd: std.posix.fd_t, data: []const u8) void {
        _ = std.c.write(fd, data.ptr, data.len);
    }

    pub fn dumpHitGrid(self: *CliRenderer) void {
        const timestamp = @divFloor(microTimestamp(), 1_000_000);
        var filename_buf: [64]u8 = undefined;
        const filename = std.fmt.bufPrint(&filename_buf, "hitgrid_{d}.txt", .{timestamp}) catch return;

        const fd = std.posix.openat(std.posix.AT.FDCWD, filename, .{ .CREAT = true, .TRUNC = true }, 0o644) catch return;
        ...
        for (0..self.hitGridHeight) |y| {
            for (0..self.hitGridWidth) |x| {
                ...
                line_buf[pos] = char;
                pos += 1;
                if (pos + 2 >= line_buf.len) {
                    fileWrite(fd, line_buf[0..pos]);
                    pos = 0;
                }
            }
            line_buf[pos] = '\n';
        ...
        if (pos > 0) fileWrite(fd, line_buf[0..pos]);
    }

    fn dumpSingleBuffer(self: *CliRenderer, buffer: *OptimizedBuffer, buffer_name: []const u8, timestamp: i64) void {
        _ = std.posix.openat(std.posix.AT.FDCWD, "buffer_dump", .{ .CREAT = true, .EXCL = true }, 0o755) catch {};
        ...
        var line_buf2: [8192]u8 = undefined;
        var pos: usize = 0;
        for (0..self.height) |y| {
            for (0..self.width) |x| {
                ...
                if (cell) |c| {
                    if (gp.isContinuationChar(c.char)) {
                        ...
                        if (bytes.len > 0) {
                            if (pos + bytes.len >= line_buf2.len) {
                                fileWrite(fd, line_buf2[0..pos]);
                                pos = 0;
                            }
                            @memcpy(line_buf2[pos..][0..bytes.len], bytes);
                            pos += bytes.len;
                        }
                    } else {
                        var utf8Buf: [4]u8 = undefined;
                        const len = std.unicode.utf8Encode(@intCast(c.char), &utf8Buf) catch 1;
                        if (pos + len >= line_buf2.len) {
                            fileWrite(fd, line_buf2[0..pos]);
                            pos = 0;
                        }
                        @memcpy(line_buf2[pos..][0..len], utf8Buf[0..len]);
                        pos += len;
                    }
                } else {
                    line_buf2[pos] = ' ';
            ...
            line_buf2[pos] = '\n';
            pos += 1;
            if (pos + 10 >= line_buf2.len) {
                fileWrite(fd, line_buf2[0..pos]);
                pos = 0;
            }
        }
        if (pos > 0) fileWrite(fd, line_buf2[0..pos]);
    }
    ...
    pub fn dumpOutputBuffer(self: *CliRenderer, timestamp: i64) void {
        ...
        w.print("Output Buffer Dump (timestamp: {d}):\n", .{timestamp}) catch return;
        w.writeAll("Last Rendered ANSI Output:\n") catch return;
        w.writeAll("================\n") catch return;

        self.backend.dumpTo(&w);

        fileWrite(fd, w.buffered());
    }
    ...
    pub fn restoreTerminalModes(self: *CliRenderer) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.restoreTerminalModes(&w) catch {};
        self.writeOut(w.buffered());
    }

    pub fn enableMouse(self: *CliRenderer, enableMovement: bool) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.setMouseMode(&w, true, enableMovement) catch {};
        self.writeOut(w.buffered());
    }
    ...
    pub fn queryThemeColors(self: *CliRenderer) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.queryThemeColors(&w) catch {};
        self.writeOut(w.buffered());
    }

    pub fn disableMouse(self: *CliRenderer) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.setMouseMode(&w, false, self.terminal.state.mouse_movement) catch {};
        self.writeOut(w.buffered());
    }

    pub fn enableKittyKeyboard(self: *CliRenderer, flags: u8) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.setKittyKeyboard(&w, true, flags) catch {};
        self.writeOut(w.buffered());
    }

    pub fn disableKittyKeyboard(self: *CliRenderer) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.setKittyKeyboard(&w, false, 0) catch {};
        self.writeOut(w.buffered());
    }
    ...
    pub fn processCapabilityResponse(self: *CliRenderer, response: []const u8) void {
        ...
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        _ = self.terminal.sendPendingQueries(&w) catch |err| blk: {
    ...
    pub fn setTerminalTitle(self: *CliRenderer, title: []const u8) void {
        var w: std.Io.Writer = .fixed(&self.writeOutBuf);
        self.terminal.setTerminalTitle(&w, title);
        self.writeOut(w.buffered());
    }
    ...
    pub fn copyToClipboardOSC52(self: *CliRenderer, target: Terminal.ClipboardTarget, text_utf8: []const u8) bool {
        ...
        var w: std.Io.Writer = .fixed(output_bytes);
        self.terminal.writeClipboard(&w, target, text_utf8) catch return false;
    ...
    pub fn clearClipboardOSC52(self: *CliRenderer, target: Terminal.ClipboardTarget) bool {
        ...
        var w: std.Io.Writer = .fixed(&clip_buf);
        self.terminal.writeClipboard(&w, target, "") catch return false;
        self.writeOut(w.buffered());
        return true;
    }
    ...
    pub fn triggerNotification(self: *CliRenderer, message: []const u8, title: ?[]const u8) bool {
        ...
        var w: std.Io.Writer = .fixed(&note_buf);

        const ok = self.terminal.writeNotification(self.allocator, &w, message, title) catch return false;
        if (!ok) return false;
        self.writeOut(w.buffered());
        return true;
    }
