                ...
                .leaf => |*l| {
                    ...
                    if (@typeInfo(T) == .@"union") {
                        ...
                        if (@hasDecl(T, "Metrics")) {
                            ...
                            try buffer.append(allocator, ':');
                            try buffer.print(allocator, "w{d}", .{metrics.weight()});

                            if (@hasDecl(T.Metrics, "total_width")) {
                                try buffer.print(allocator, ",tw{d}", .{metrics.custom.total_width});
                            }
                            if (@hasDecl(T.Metrics, "total_bytes")) {
                                try buffer.print(allocator, ",b{d}", .{metrics.custom.total_bytes});
                            }
                        }
