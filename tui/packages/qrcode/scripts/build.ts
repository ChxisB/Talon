...
console.log("Building @tui/qrcode library...")
...
if (existsSync(corePackageJsonPath)) {
  console.log("Ensuring @tui/core declarations are up to date...")
  ...
  if (coreBuildResult.status !== 0) {
    console.error("Error: Failed to build @tui/core declarations required by @tui/qrcode")
    process.exit(1)
  }
}
...
if (processedDependencies["@tui/core"] === "workspace:*") {
  processedDependencies["@tui/core"] = packageJson.version
}

const processedPeerDependencies = { ...packageJson.peerDependencies }
for (const dependencyName of ["@tui/react", "@tui/solid"]) {
