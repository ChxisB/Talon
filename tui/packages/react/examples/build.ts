...
const workspaceAliasPlugin: BunPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@tui\/react$/ }, () => {
      return {
        path: join(packageRoot, "src", "index.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/react\/runtime-plugin-support$/ }, () => {
      return {
        path: join(packageRoot, "scripts", "runtime-plugin-support.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/react\/runtime-plugin-support\/configure$/ }, () => {
      return {
        path: join(packageRoot, "scripts", "runtime-plugin-support-configure.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/react\/jsx-runtime$/ }, () => {
      return {
        path: join(packageRoot, "jsx-runtime.js"),
      }
    })

    build.onResolve({ filter: /^@tui\/react\/jsx-dev-runtime$/ }, () => {
      return {
        path: join(packageRoot, "jsx-dev-runtime.js"),
      }
    })

    build.onResolve({ filter: /^@tui\/core$/ }, () => {
      return {
        path: join(packageRoot, "..", "core", "src", "index.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/core\/extras$/ }, () => {
      return {
        path: join(packageRoot, "..", "core", "src", "extras.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/core\/3d$/ }, () => {
      return {
        path: join(packageRoot, "..", "core", "src", "3d.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/core\/testing$/ }, () => {
      return {
        path: join(packageRoot, "..", "core", "src", "testing.ts"),
      }
    })

    build.onResolve({ filter: /^@tui\/core\/runtime-plugin$/ }, () => {
      return {
