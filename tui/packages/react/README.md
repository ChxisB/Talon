...
```bash
bun install @tui/react @tui/core react
```
...
QR code support is available from `@tui/qrcode/react` and must be registered explicitly with `registerQRCode()`.
...
```tsx
import { useRenderer } from "@tui/react"
...
```tsx
import { useKeyboard } from "@tui/react"
...
```tsx
import { decodePasteBytes } from "@tui/core"
import { usePaste } from "@tui/react"
...
- `handler`: Callback function that receives a `PasteEvent` object with `bytes: Uint8Array` (decode with `decodePasteBytes` from `@tui/core`)
...
```tsx
import { useFocus } from "@tui/react"
...
```tsx
import { useSelectionHandler } from "@tui/react"
...
```tsx
import { useOnResize, useRenderer } from "@tui/react"
...
```tsx
import { useTerminalDimensions } from "@tui/react"
...
```tsx
import { useTimeline } from "@tui/react"
...
```tsx
import type { TextareaRenderable } from "@tui/core"
import { useKeyboard, useRenderer } from "@tui/react"
...
```tsx
import type { SelectOption } from "@tui/core"
import { useState } from "react"
...
```tsx
import { RGBA, SyntaxStyle } from "@tui/core"
...
```tsx
import type { LineNumberRenderable } from "@tui/core"
import { RGBA, SyntaxStyle } from "@tui/core"
import { useEffect, useRef } from "react"
...
```tsx
import { createCliRenderer } from "@tui/core"
import { createRoot, useKeyboard } from "@tui/react"
...
import { createRoot, useTimeline } from "@tui/react"
import { useEffect, useState } from "react"
...
import { createRoot, extend } from "@tui/react"
