...
const defaultRuntimeModules: Record<string, RuntimeModuleEntry> = {
  "@tui/react": opentuiReactRuntime as Record<string, unknown>,
  "@tui/react/jsx-runtime": reactJsxRuntime as Record<string, unknown>,
  "@tui/react/jsx-dev-runtime": reactJsxDevRuntime as Record<string, unknown>,
...
function assertCompatibleInstall(
  ...
  for (const specifier of Object.keys(modules)) {
    if (!install.specifiers.has(specifier)) {
      throw new Error(
        `OpenTUI React runtime plugin support is already installed without ${specifier}. Call ensureRuntimePluginSupport({ additional }) from @tui/react/runtime-plugin-support/configure before importing @tui/react/runtime-plugin-support.`,
      )
    }
  }
