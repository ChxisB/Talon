...
try {
  ...
  const distPackage = JSON.parse(readFileSync(join(root, "dist/package.json"), "utf8")) as {
    peerDependencies: { "@tui/core": string }
  }
  ...
  writeFileSync(
    ...
    JSON.stringify({
      name: "@tui/core",
      version: distPackage.peerDependencies["@tui/core"],
      type: "module",
      types: "./index.d.ts",
  ...
  writeFileSync(
    ...
    JSON.stringify({
      ...
      type: "module",
      dependencies: {
        "@tui/core": "file:./core-stub",
        "@tui/ssh": `file:${tarball}`,
        "@types/node": "^24.0.0",
        ssh2: "^1.16.0",
        typescript: "^5",
      },
  ...
  writeFileSync(
    ...
    `import { createServer, logging, ConfigError } from "@tui/ssh"
...
`,
  ...
  writeFileSync(
    ...
    'import { createServer, type ListenInfo } from "@tui/ssh"; const server = createServer().serve(() => {}); const listen: Promise<ListenInfo> = server.listen(0); void listen\n',
  )
