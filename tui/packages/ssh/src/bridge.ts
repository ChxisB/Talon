...
import { CliRenderEvents, createCliRenderer, type CliRenderer } from "@tui/core"
...
export function createSessionBridge(channel: ServerChannel, options: SessionBridgeOptions): SessionBridge {
  ...
  const session: Session & MiddlewareSession = {
    get renderer() {
      if (!renderer) {
        throw new Error(
          "@tui/ssh: session.renderer is unavailable until the handler runs — a middleware must call next() before using it",
        )
      }
      return renderer
    },
