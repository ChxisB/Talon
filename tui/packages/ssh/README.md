...
`@tui/ssh` turns an incoming SSH session into a fully-wired OpenTUI
...
package is **renderer-agnostic**: it depends only on `@tui/core`, never on
`@tui/react` or `@tui/solid`, so the same server works with all three.

```ts
import { createServer } from "@tui/ssh"
import { BoxRenderable, TextRenderable } from "@tui/core"
...
`@tui/core` is a peer dependency. Supported runtimes are Bun ≥ 1.3.0 and
...
> `@tui/react` / `@tui/solid` are **not** runtime dependencies of this
...
> Publickey auth is verified end-to-end: `@tui/ssh` checks the client's
...
```ts
import { createServer, type Middleware } from "@tui/ssh"
...
`@tui/ssh` ships one ready-made middleware. `logging()` is a setup/teardown
...
```ts
import { createServer, logging } from "@tui/ssh"
