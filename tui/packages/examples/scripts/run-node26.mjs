...
function prepareCorePackage() {
  ...
  const sourceNativeDir = resolve(coreRoot, "node_modules", "@tui", nativePackageName)
  const targetNativeDir = resolve(packageRoot, "node_modules", "@tui", nativePackageName)

  run("bun", ["run", "build"], coreRoot)

  mkdirSync(resolve(packageRoot, "node_modules", "@tui"), { recursive: true })
...
function copyCoreDistPackage() {
  const targetCoreDir = resolve(bundleDir, "node_modules", "@tui", "core")
