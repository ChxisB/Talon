...
const workspaceAliasPlugin: BunPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^@tui\/core-(?:darwin|linux|win32)-/ }, (args) => {
    ...
    build.onResolve({ filter: /^@tui\/three$/ }, () => ({
      path: join(threeRoot, "src", "index.ts"),
    }))

    build.onResolve({ filter: /^@tui\/keymap$/ }, () => ({
      path: join(keymapRoot, "src", "index.ts"),
    }))

    build.onResolve({ filter: /^@tui\/qrcode$/ }, () => ({
      path: join(qrcodeRoot, "src", "index.ts"),
    }))

    build.onResolve({ filter: /^@tui\/keymap\/addons\/opentui$/ }, () => ({
      path: join(keymapRoot, "src", "addons", "opentui", "index.ts"),
    }))

    build.onResolve({ filter: /^@tui\/keymap\/opentui$/ }, () => ({
...
function getNativePackageDir(platform: string, arch: string): string {
  ...
  return join(coreRoot, "node_modules", "@tui", `core-${packagePlatform}-${arch}`)
}
