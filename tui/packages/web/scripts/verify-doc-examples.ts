 ...
 * 2. Type-checks them against @tui/core
...
function wrapCodeForTypeCheck(code: string, blockIndex: number): string {
  ...
  if (importedModules.some((module) => !module.startsWith("@tui/core"))) {
    return ""
  }
  ...
  if (usesRenderer && !definesRenderer) {
    ...
    if (!preamble.includes("createCliRenderer")) {
      preamble = `import { createCliRenderer } from "@tui/core"\n` + preamble
    }
...
async function setupTestEnv(): Promise<boolean> {
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        name: corePackageJson.name ?? "@tui/core",
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      type: "module",
      dependencies: {
        "@tui/core": `file:${VENDORED_CORE_PACKAGE}`,
      },
    }),
  )
