 ...
 *
 * This creates a directory with @tui/core installed where you can
...
async function main() {
  ...
  // Install @tui/core
  console.log("Installing @tui/core...")
  const install = Bun.spawnSync(["bun", "add", "@tui/core"], {
    cwd: targetDir,
    stdout: "pipe",
    stderr: "pipe",
  })
  ...
  // install @tui/solid
  console.log("Installing @tui/solid...")
  const installSolid = Bun.spawnSync(["bun", "add", "@tui/solid"], {
    cwd: targetDir,
    stdout: "pipe",
    stderr: "pipe",
  })
  ...
  const template = `import { createCliRenderer, Text, Box } from "@tui/core"
