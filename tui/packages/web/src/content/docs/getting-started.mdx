...
OpenTUI's renderer examples use [Bun](https://bun.sh). You can import portable entry points such as `@tui/keymap` and
`@tui/core` in Node.js without installing Bun or enabling FFI. These imports do not create a native renderer. To
...
`await import("@tui/keymap")` and `await import("@tui/core")` without `--experimental-ffi`.
...
```typescript
import { createCliRenderer, Text } from "@tui/core"
...
```typescript
import { createCliRenderer, Box, Text } from "@tui/core"
...
The OpenTUI monorepo includes `@tui/react` and `@tui/solid`. Other language and framework bindings live in
