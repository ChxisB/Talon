...
```ts
import type { Command, KeymapEvent } from "@tui/keymap"
...
| Package                          | Description                                                                |
| -------------------------------- | -------------------------------------------------------------------------- |
| `@tui/keymap`                | Main engine entry: `Keymap`, key stringifiers and shared types             |
| `@tui/keymap/addons`         | Universal addons for parser stages, metadata, diagnostics and sequences    |
| `@tui/keymap/addons/opentui` | Universal addons plus OpenTUI-specific base-layout and edit-buffer helpers |
| `@tui/keymap/extras`         | Pure config and formatting helpers                                         |
| `@tui/keymap/extras/graph`   | Graph snapshot helpers for debug and graph UIs                             |
| `@tui/keymap/testing`        | Host-agnostic fake host and diagnostics for addon tests                    |
| `@tui/keymap/opentui`        | OpenTUI host adapter for terminal apps built on `@tui/core`            |
| `@tui/keymap/html`           | DOM host adapter for browser UIs rooted in an `HTMLElement`                |
| `@tui/keymap/react`          | React provider and hooks for an OpenTUI keymap                             |
| `@tui/keymap/solid`          | Solid provider and hooks for an OpenTUI keymap                             |

Adapter entry points intentionally export adapter-specific helpers. Import the shared `Keymap`, stringifiers and core types from `@tui/keymap`.
