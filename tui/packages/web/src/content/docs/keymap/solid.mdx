...
This page covers `@tui/keymap/solid`.

These bindings are for OpenTUI Solid apps. They consume a pre-created `Keymap<Renderable, KeyEvent>` from `@tui/keymap/opentui`; they do not wrap the DOM/HTML adapter used in browser Solid apps.
...
import { KeymapProvider, useBindings } from "@tui/keymap/solid"
