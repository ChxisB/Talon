...
- `@tui/keymap/opentui` for terminal apps built on `CliRenderer` and `Renderable`
- `@tui/keymap/html` for browser UIs rooted in an `HTMLElement`
...
Both built-in host packages export adapter-specific helpers. Import the shared `Keymap`, stringifiers and core types from `@tui/keymap`.

| Package                   | Host adapter                        | Bare keymap helper              | Default helper                         |
| ------------------------- | ----------------------------------- | ------------------------------- | -------------------------------------- |
| `@tui/keymap/opentui` | `createOpenTuiKeymapHost(renderer)` | `createOpenTuiKeymap(renderer)` | `createDefaultOpenTuiKeymap(renderer)` |
| `@tui/keymap/html`    | `createHtmlKeymapHost(root)`        | `createHtmlKeymap(root)`        | `createDefaultHtmlKeymap(root)`        |
...
`@tui/keymap/opentui` is the host package for terminal apps built on `@tui/core`.
...
It does not install leader tokens, ex commands, disambiguation addons, warning analyzers, base-layout fallback or textarea helpers. Call `registerBaseLayoutFallback()` from `@tui/keymap/addons/opentui` if you want bindings to ignore active keyboard-layout changes.
...
`@tui/keymap/addons/opentui` adds helpers on top of the shared addon set:
...
`@tui/keymap/html` is the host package for browser UIs rooted in an `HTMLElement` subtree.
...
```ts
import { createDefaultHtmlKeymap } from "@tui/keymap/html"
...
```ts
import { Keymap, type KeymapHost } from "@tui/keymap"
