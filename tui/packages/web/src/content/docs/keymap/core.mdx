...
This page covers `@tui/keymap` without any host-specific adapter.
...
The core engine only accepts full `Binding[]` arrays. If you want command-to-key config sugar, use `commandBindings(map)` from `@tui/keymap/extras` to build the array before calling `registerLayer()`.
...
Use `stringifyKeyStroke()` or `stringifyKeySequence()` for display. Canonical formatting renders `return` as `enter`; `preferDisplay: true` keeps preserved token displays such as `<leader>`. Both are exported from `@tui/keymap`:

```ts
import { stringifyKeyStroke, stringifyKeySequence } from "@tui/keymap"
```
...
Pure extra exported from `@tui/keymap/extras`:
...
```ts
import { formatCommandBindings, formatKeySequence } from "@tui/keymap/extras"
...
`getGraphSnapshot(keymap, options?)` from `@tui/keymap/extras/graph` returns a diagnostic projection for graph visualizers and debug UIs. It includes layers, commands, bindings, sequence nodes, active keys and the current pending sequence.
...
```ts
import { getGraphSnapshot } from "@tui/keymap/extras/graph"
