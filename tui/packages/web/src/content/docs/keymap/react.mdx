...
This page covers `@tui/keymap/react`.

These bindings are for OpenTUI React apps. They consume a pre-created `Keymap<Renderable, KeyEvent>` from `@tui/keymap/opentui`; they do not wrap the DOM/HTML adapter used in browser React apps.
...
import { KeymapProvider, useBindings } from "@tui/keymap/react"
