...
`@tui/solid` publishes one npm package with runtime-specific entrypoints behind the package `exports` map. The root package and JSX runtime subpaths work in both Bun and Node. Bun-specific helpers stay Bun-only and throw a clear runtime error if imported from Node.

| Entrypoint                                        | Bun | Node | Notes                                           |
| ------------------------------------------------- | --- | ---- | ----------------------------------------------- |
| `@tui/solid`                                  | yes | yes  | Main renderer and test helpers                  |
| `@tui/solid/jsx-runtime`                      | yes | yes  | Real runtime module for compiled JSX            |
| `@tui/solid/jsx-dev-runtime`                  | yes | yes  | Real dev runtime module for compiled JSX        |
| `@tui/solid/preload`                          | yes | no   | Bun preload hook                                |
| `@tui/solid/bun-plugin`                       | yes | no   | Bun bundler/plugin entry                        |
| `@tui/solid/runtime-plugin-support`           | yes | no   | Bun runtime-loader support                      |
| `@tui/solid/runtime-plugin-support/configure` | yes | no   | Bun runtime-loader support without side effects |
...
export default {
  ...
  presets: [[solid, { moduleName: "@tui/solid", generate: "universal" }], [ts]],
}
```
...
```ts
import "@tui/solid/runtime-plugin-support"
```
...
```tsx
import { writeSolidToScrollback } from "@tui/solid"
...
```tsx
import { onResize } from "@tui/solid"
...
```tsx
import { usePaste } from "@tui/solid"
...
```tsx
import { useSelectionHandler } from "@tui/solid"
...
```tsx
import { useTimeline } from "@tui/solid"
...
```tsx
import { Portal, useRenderer } from "@tui/solid"
...
If that executable loads external plugins/modules at runtime, keep `import "@tui/solid/runtime-plugin-support"` in your app entry.
