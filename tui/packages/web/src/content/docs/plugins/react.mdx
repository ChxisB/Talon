...
This installs Bun runtime support so external TS/TSX plugin modules resolve against the host runtime instances (`@tui/react`, React JSX runtime modules, and core runtime modules).
...
```ts
import { ensureRuntimePluginSupport } from "@tui/react/runtime-plugin-support/configure"
...
First-party packages expose side-effect-free runtime module maps. For plugins that import `@tui/three`, install `@tui/three` in the host app and pass its runtime modules explicitly:
...
- `@tui/react/runtime-plugin-support` — one-line runtime support for external plugin/module loading
- `@tui/react/runtime-plugin-support/configure` — configurable runtime support without import-time side effects
...
import { createReactSlotRegistry, createRoot, Slot } from "@tui/react"
