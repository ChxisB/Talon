...
- `@tui/core/runtime-plugin-support` / `createRuntimePlugin` — runtime module support for external plugin/module loading in Bun
...
```ts
import "@tui/core/runtime-plugin-support"
```

This installs Bun runtime support for `@tui/core` plus default core runtime entrypoints (`@tui/core/testing`). Add first-party package runtime modules explicitly when a plugin imports them.

If you need additional host-resolved modules, configure runtime support before the side-effect import is loaded:

```ts
const { ensureRuntimePluginSupport } = await import("@tui/core/runtime-plugin-support/configure")
...
For plugins that import `@tui/three`, install `@tui/three` in the host app and pass its runtime module map:

```ts
import { runtimeModules as threeRuntimeModules } from "@tui/three/runtime-modules"
import { ensureRuntimePluginSupport } from "@tui/core/runtime-plugin-support/configure"
...
```typescript
import { createCliRenderer, createCoreSlotRegistry, resolveCoreSlot } from "@tui/core"
