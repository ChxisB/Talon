...
```typescript
import { createCliRenderer, TextRenderable, BoxRenderable } from "@tui/core"
...
QR code support is available from the separate `@tui/qrcode` package.
...
```typescript
import { RenderableEvents } from "@tui/core"
