# Data Flow Renderer (Reference — JS)

Render `diagram_type: "dataflow"` JSON files into the standalone HTML template.
...
The JS renderer validates input against `schemas/dataflow.schema.json` via ajv.
...
or falls back to `dataflow.html`.
...
A complete worked example lives at `examples/product-analytics.dataflow.json`.

The schema lives at `schemas/dataflow.schema.json`.
