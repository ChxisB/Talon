# Lifecycle Renderer (Reference — JS)

Render `diagram_type: "lifecycle"` JSON files into the standalone HTML template.
...
The JS renderer validates input against `schemas/lifecycle.schema.json` via ajv.
...
or falls back to `lifecycle.html`.
...
lane id shares the single middle event band. A complete worked example lives at
`examples/agent-run.lifecycle.json`.

The schema lives at `schemas/lifecycle.schema.json`.
