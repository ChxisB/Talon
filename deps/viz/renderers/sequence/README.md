# Sequence Renderer (Reference — JS)

Render `diagram_type: "sequence"` JSON files into the standalone HTML template.
...
The JS renderer validates input against `schemas/sequence.schema.json` via ajv.
...
or falls back to `sequence.html`.
...
complete worked example lives at `examples/cache-miss-request.sequence.json`.

The schema lives at `schemas/sequence.schema.json`.
