...
// with reason=tool_use and used to terminate `talon run` early via
...
// subscribers (e.g. a TUI watching workspace events while talon
...
// non-nil error from `talon run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `talon run` waits) exits cleanly rather
...
// queue scenario: `talon run --continue` attaches to a session
