...
// event into a closure nobody reads, and subscribers (`talon run`)
...
// publish a RunComplete for its RunID, hanging a `talon run` caller that
...
// than internal bookkeeping, which is the contract a `talon run` caller
...
// bookkeeping slice, not the published event a `talon run` caller awaits.
