...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent, isSubAgent bool) ([]llm.AgentTool, error) {
	...
	allTools = append(
		...
		tools.NewScreenshotTool(tools.ScreenshotConfig{
			VisionEndpoint: c.cfg.Config().Tools.Vision.EndpointOrDefault(),
			VisionModel:    c.cfg.Config().Tools.Vision.ModelOrDefault(),
		}),
		tools.NewAnalyzeImageTool(tools.VisionConfig{
			Endpoint: c.cfg.Config().Tools.Vision.EndpointOrDefault(),
			Model:    c.cfg.Config().Tools.Vision.ModelOrDefault(),
		}),
