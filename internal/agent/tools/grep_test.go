...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	talonignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".talonignore"), []byte(talonignoreContent), 0o644))
	...
	for name, fn := range map[string]func(pattern, path, include string) ([]grepMatch, error){
		...
		t.Run(name, func(t *testing.T) {
			...
			require.False(t, foundFiles["lib.js"], "Should not find lib.js (ignored by .talonignore)")
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".talonignore"), []byte("file5.txt\n"), 0o644))
