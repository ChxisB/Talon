...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "talon.log")
...
func TestNewtalonLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewtalonLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, talonLogsToolName, tool.Info().Name)
}

func TesttalonLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 3})
...
func TesttalonLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 0})
...
func TesttalonLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TesttalonLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runtalonLogs("/nonexistent/path/talon.log", talonLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TesttalonLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "talon.log")
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TesttalonLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "talon.log")
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 10})
...
func TesttalonLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 1})
...
func TesttalonLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TesttalonLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TesttalonLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TesttalonLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "talon.log")
...
func TesttalonLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "talon.log")
...
func TesttalonLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TesttalonLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TesttalonLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TesttalonLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runtalonLogs(logFile, talonLogsParams{Lines: 5})
...
func TesttalonLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
