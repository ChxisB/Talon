...
const TalonLogsToolName = "talon_logs"

//go:embed talon_logs.md.tpl
var talonLogsDescriptionTmpl []byte

var talonLogsDescriptionTpl = template.Must(
	template.New("talonLogsDescription").
		Parse(string(talonLogsDescriptionTmpl)),
)

type talonLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func talonLogsDescription() string {
	return renderTemplate(talonLogsDescriptionTpl, talonLogsDescriptionData{
...
type TalonLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewTalonLogsTool(logFile string) llm.AgentTool {
	return llm.NewAgentTool(
		TalonLogsToolName,
		talonLogsDescription(),
		func(ctx context.Context, params TalonLogsParams, call llm.ToolCall) (llm.ToolResponse, error) {
			result := runTalonLogs(logFile, params)
			return llm.NewTextResponse(result), nil
		},
	)
}

// runTalonLogs reads and formats the last N log entries from the given file.
func runTalonLogs(logFile string, params TalonLogsParams) string {
