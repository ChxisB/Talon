...
func makeStep(calls []llm.ToolCallContent, results []llm.ToolResultContent) llm.StepResult {
	var content llm.ResponseContent
	for _, c := range calls {
...
func makeToolStep(name, input, output string) llm.StepResult {
	...
	return makeStep(
		...
		[]llm.ToolResultContent{
			{ToolCallID: callID, ToolName: name, Result: llm.ToolResultOutputContentText{Text: output}},
		},
	)
}

// makeEmptyStep creates a step with no tool calls (e.g. a text-only response).
func makeEmptyStep() llm.StepResult {
	return llm.StepResult{
		Response: llm.Response{
...
func TestGetToolInteractionSignature(t *testing.T) {
	t.Run("empty content returns empty string", func(t *testing.T) {
		sig := getToolInteractionSignature(llm.ResponseContent{})
		if sig != "" {
	...
	t.Run("tool call with result produces signature", func(t *testing.T) {
		content := llm.ResponseContent{
			llm.ToolCallContent{ToolCallID: "1", ToolName: "read", Input: `{"file":"a.go"}`},
			llm.ToolResultContent{ToolCallID: "1", ToolName: "read", Result: llm.ToolResultOutputContentText{Text: "content"}},
		}
		sig := getToolInteractionSignature(content)
		if sig == "" {
	...
	t.Run("same interactions produce same signature", func(t *testing.T) {
		content1 := llm.ResponseContent{
			llm.ToolCallContent{ToolCallID: "1", ToolName: "read", Input: `{"file":"a.go"}`},
			llm.ToolResultContent{ToolCallID: "1", ToolName: "read", Result: llm.ToolResultOutputContentText{Text: "content"}},
		}
		content2 := llm.ResponseContent{
			llm.ToolCallContent{ToolCallID: "2", ToolName: "read", Input: `{"file":"a.go"}`},
			llm.ToolResultContent{ToolCallID: "2", ToolName: "read", Result: llm.ToolResultOutputContentText{Text: "content"}},
		}
	...
	t.Run("different inputs produce different signatures", func(t *testing.T) {
		...
		content2 := llm.ResponseContent{
			llm.ToolCallContent{ToolCallID: "1", ToolName: "read", Input: `{"file":"b.go"}`},
			llm.ToolResultContent{ToolCallID: "1", ToolName: "read", Result: llm.ToolResultOutputContentText{Text: "content"}},
		}
		sig1 := getToolInteractionSignature(content1)
		sig2 := getToolInteractionSignature(content2)
