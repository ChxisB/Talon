---
name: talon-config
description: Use when the user needs help configuring Talon — working with talon.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Talon behavior.
---

# Talon Configuration

Talon uses JSON configuration files with the following priority (highest to lowest):

1. `.talon.json` (project-local, hidden)
2. `talon.json` (project-local)
3. `$XDG_CONFIG_HOME/talon/talon.json` or `$HOME/.config/talon/talon.json` (global)

## Basic Structure

```json
{
  "$schema": "https://github.com/ChxisB/talon/blob/main/talon.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
Talon runs selected string fields through an embedded bash-compatible
...
`talon.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Talon in a directory whose `talon.json` you haven't
reviewed.
...
> `.agents/skills`, `.talon/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".talon/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".talon/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `TALON_EVENT` | Event name (e.g. `PreToolUse`) |
| `TALON_TOOL_NAME` | Name of the tool being called |
| `TALON_SESSION_ID` | Current session ID |
| `TALON_CWD` | Current working directory |
| `TALON_PROJECT_DIR` | Project root directory |
| `TALON_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `TALON_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Talon also supports the Claude Code hook output format:
...
- `TALON_GLOBAL_CONFIG` - Override global config location
- `TALON_GLOBAL_DATA` - Override data directory location
- `TALON_SKILLS_DIR` - Override default skills directory
