---
name: talon-hooks
description: Use when the user wants to add, write, debug, or configure a Talon hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in talon.json.
---

# Talon Hooks

Hooks are user-defined commands in `talon.json` that fire at specific points
...
| `TALON_TOOL_NAME`            | Tool being called (e.g. `bash`)          |
| `TALON_SESSION_ID`           | Current session ID                       |
| `TALON_CWD`                  | Working directory                        |
| `TALON_PROJECT_DIR`          | Project root directory                   |
| `TALON_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command      |
| `TALON_TOOL_INPUT_FILE_PATH` | For file tools: the target file path     |
...
if echo "$TALON_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
if [[ "$TALON_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
3. Add the entry under `hooks.PreToolUse` in `talon.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check Talon logs.
...
Talon also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Talon treats `updated_input` as shallow-merge, Claude Code replaces.
