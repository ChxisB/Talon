...
func New(ctx context.Context, conn *sql.DB, store *config.ConfigStore, skillsMgr *skills.Manager) (*App, error) {
	...
	// Only initialize coder agent if a model has been explicitly selected.
	// On first run, the model dialog handles this after user selection.
	if !cfg.HasUserSelectedModel() {
		slog.Warn("No model selected yet, deferring agent initialization")
		return app, nil
	}
