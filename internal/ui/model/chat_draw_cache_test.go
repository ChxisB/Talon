...
func drawTestArea(width, height int) term.Rectangle {
	return term.Rect(0, 0, width, height)
}

// renderToBuffer mirrors what Chat.Draw does at the screen layer so tests
// can assert byte-equivalence with a fresh term.NewStyledString render.
func renderToBuffer(t *testing.T, c *Chat, w, h int) string {
	t.Helper()
	scr := term.NewScreenBuffer(w, h)
...
// term.NewStyledString render byte-for-byte.
func TestChatDrawCache_MissOnDifferentRender(t *testing.T) {
	...
	// Output must match a fresh term.NewStyledString render of the current
...
// term.NewStyledString render against the new area.
func TestChatDrawCache_ReusedAcrossDifferentArea(t *testing.T) {
	...
	smallScr := term.NewScreenBuffer(40, 10)
	u.chat.Draw(smallScr, term.Rect(0, 0, 40, 10))
...
// freshStyledRender renders the same string through term.NewStyledString into
...
func freshStyledRender(s string, w, h int) string {
	scr := term.NewScreenBuffer(w, h)
	term.NewStyledString(s).Draw(scr, term.Rect(0, 0, w, h))
	return scr.Render()
}
...
func TestChatDrawCache_InvalidatedByWidthMethodSwap(t *testing.T) {
	...
	scrA := term.ScreenBuffer{
		RenderBuffer: term.NewRenderBuffer(w, h),
		Method:       ansi.GraphemeWidth,
	}
	u.chat.Draw(scrA, term.Rect(0, 0, w, h))
	require.NotNil(t, u.chat.drawCache)
	...
	scrB := term.ScreenBuffer{
		RenderBuffer: term.NewRenderBuffer(w, h),
		Method:       ansi.WcWidth,
	}
	u.chat.Draw(scrB, term.Rect(0, 0, w, h))
	require.NotSame(t, firstCache, u.chat.drawCache,
		"width method change must invalidate the cache")
	require.Equal(t, ansi.WcWidth, u.chat.drawCache.method)
}

// fixedMethodScreen wraps a term.ScreenBuffer but reports a custom (non
...
type fixedMethodScreen struct {
	term.ScreenBuffer
	method term.WidthMethod
}

func (s fixedMethodScreen) WidthMethod() term.WidthMethod { return s.method }
...
// and falls through to a direct term.NewStyledString.Draw, producing the
...
func TestChatDrawCache_FallbackOnNonAnsiMethod(t *testing.T) {
	...
	scr := fixedMethodScreen{
		ScreenBuffer: term.ScreenBuffer{
			RenderBuffer: term.NewRenderBuffer(w, h),
			Method:       ansi.GraphemeWidth,
		},
		method: customWidth{},
	...
	u.chat.Draw(scr, term.Rect(0, 0, w, h))

	// The fallback path uses term.NewStyledString(rendered).Draw — same
...
func TestRenderedBounds_MatchesPrintStringTallyForZWJ(t *testing.T) {
	...
	for _, m := range []ansi.Method{ansi.GraphemeWidth, ansi.WcWidth} {
		...
		scr := term.ScreenBuffer{
			RenderBuffer: term.NewRenderBuffer(64, 4),
			Method:       m,
		}
		term.NewStyledString(sample).Draw(scr, term.Rect(0, 0, 64, 4))
