...
// imageIndex is a per-assistant-message counter for image placeholders.
// It resets when the message ID changes (new assistant message being rendered).
var (
	imageIndex    int
	lastMessageID string
)

// toolOutputImageContent renders an image placeholder like "[image 1]"
// or "[screenshot 1]" with media type and size info. It uses the tool call
// ID to auto-reset the counter when a new message starts rendering.
func toolOutputImageContent(sty *styles.Styles, data, mediaType, toolName, msgID string) string {
	...
	return sty.Tool.Body.Render(fmt.Sprintf(
		"%s  %s %s  %s",
		sty.Tool.ResourceLoadedText.Render(fmt.Sprintf("[%s %d]", label, imageIndex)),
...
func roundedEnumerator(lPadding, width int) styletree.Enumerator {
	if width == 0 {
		width = 2
	}
	if lPadding == 0 {
	...
	return func(children styletree.Children, index int) string {
